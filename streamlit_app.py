...
- sobreposição multicritério de NU_NOTIFIC/NM_PACIENT, com comparação par a par de identidade, datas e classificação etiológica.
...
def _sinan_quadro_mapping(numero: str) -> Dict[str, str]:
    for quadro in QUADROS_ETIOLOGICOS_SINAN:
...
SINAN_DIAGNOSTIC_RESULT_MAPPINGS: Dict[str, Dict[str, str]] = {
    "Cultura Líquor": _sinan_quadro_mapping("VII"),
    "Cultura Sangue": _sinan_quadro_mapping("VII"),
    "Bacterioscopia líquor": _sinan_quadro_mapping("VIII"),
    "CIE líquor": _sinan_quadro_mapping("IX"),
    "LÁTEX líquor": _sinan_quadro_mapping("X"),
    "Isolamento viral líquor": _sinan_quadro_mapping("XI"),
    "PCR líquor": _sinan_quadro_mapping("XII"),
}
SINAN_CRITERION_DICTIONARY_MAPPING = _sinan_quadro_mapping("V")

SINAN_DIAGNOSTIC_METHOD_SPECS: Dict[str, Dict[str, object]] = {
    "Clínico": {"expr_key": "criterio_code", "criterion_code": "04", "dictionary_source": "Quadro V — CRITERIO"},
    "Quimiocitológico": {"expr_key": "criterio_code", "criterion_code": "06", "dictionary_source": "Quadro V — CRITERIO"},
    "Aspecto do Líquor": {
        "expr_key": "lab_aspect_code",
        "mapping": {"1": "Límpido", "2": "Purulento", "3": "Hemorrágico", "4": "Turvo", "5": "Xantocrômico", "6": "Outro", "9": "Ignorado"},
        "not_informative": {"9"},
        "dictionary_source": "Campo 48",
    },
    "Cultura Líquor": {"expr_key": "lab_ctliqu_code", "mapping": SINAN_DIAGNOSTIC_RESULT_MAPPINGS["Cultura Líquor"], "not_informative": {"61", "62"}, "dictionary_source": "Quadro VII"},
    "Cultura Sangue": {"expr_key": "lab_ctsang_code", "mapping": SINAN_DIAGNOSTIC_RESULT_MAPPINGS["Cultura Sangue"], "not_informative": {"61", "62"}, "dictionary_source": "Quadro VII"},
    "Bacterioscopia líquor": {"expr_key": "lab_bcliqu_code", "mapping": SINAN_DIAGNOSTIC_RESULT_MAPPINGS["Bacterioscopia líquor"], "not_informative": {"61", "62"}, "dictionary_source": "Quadro VIII"},
    "CIE líquor": {"expr_key": "lab_ciliqu_code", "mapping": SINAN_DIAGNOSTIC_RESULT_MAPPINGS["CIE líquor"], "not_informative": {"61", "62"}, "dictionary_source": "Quadro IX"},
    "LÁTEX líquor": {"expr_key": "lab_agliqu_code", "mapping": SINAN_DIAGNOSTIC_RESULT_MAPPINGS["LÁTEX líquor"], "not_informative": {"61", "62"}, "dictionary_source": "Quadro X"},
    "Isolamento viral líquor": {"expr_key": "lab_isliqu_code", "mapping": SINAN_DIAGNOSTIC_RESULT_MAPPINGS["Isolamento viral líquor"], "not_informative": set(), "dictionary_source": "Quadro XI"},
    "PCR líquor": {"expr_key": "lab_pcliqu_code", "mapping": SINAN_DIAGNOSTIC_RESULT_MAPPINGS["PCR líquor"], "not_informative": {"61", "62"}, "dictionary_source": "Quadro XII"},
}
...
# Quadros VII e XII. Categorias amplas (p.ex. "outras bactérias") são excluídas.
SINAN_BACTERIOSCOPY_PRECISE_AGENT = {
    "03": "01",  # diplococos Gram-negativos -> N. meningitidis
    "08": "07",  # diplococos Gram-positivos -> S. pneumoniae
    "36": "06",  # cocobacilos -> H. influenzae
}

SINAN_PRECISE_BACTERIAL_AGENT_LABELS = {
    "01": "Neisseria meningitidis",
    "06": "Haemophilus influenzae",
    "07": "Streptococcus pneumoniae",
}
...
# Parâmetros solicitados para a auditoria literal de completude. A ordem é a
# mesma apresentada ao usuário e não inclui qualquer regra de plausibilidade:
# nessa seção, toda célula não vazia é um preenchimento, exatamente como veio.
SINAN_LCR_COMPLETENESS_PARAMS = {
    "Glicose": "glico",
    "Proteínas": "prot",
    "Leucócitos": "leuco",
    "Neutrófilos": "neutro",
...
#
# ATENÇÃO: os códigos sentinela e tetos formam uma camada operacional opcional,
# desligada por padrão; devem ser validados contra o dicionário oficial do SINAN
# da versão em uso antes de relatórios formais.
...
SINAN_LCR_QUALITY_TOGGLE_KEY = "sinan_lcr_apply_quality_criteria"


def sinan_lcr_quality_criteria_enabled(explicit: Optional[bool] = None) -> bool:
    """Retorna o modo analítico escolhido pelo usuário.

    O padrão é sempre False para preservar os valores registrados. O estado só
    passa a True depois de uma ação explícita no controle exibido no bloco do
    LCR. ``explicit`` existe para testes e chamadas puras que não dependam da UI.
    """
    if explicit is not None:
        return bool(explicit)
    return bool(st.session_state.get(SINAN_LCR_QUALITY_TOGGLE_KEY, False))
...
def sinan_lcr_neutralize_sentinel_expr(
    value_expr: str,
    *,
    apply_quality_criteria: Optional[bool] = None,
) -> str:
    """Mantém o bruto por padrão e neutraliza sentinelas somente sob opt-in."""
    if not sinan_lcr_quality_criteria_enabled(apply_quality_criteria):
        return f"({value_expr})"
...
def sinan_lcr_clean_value_expr(
    value_expr: str,
    param_key: Optional[str] = None,
    *,
    apply_quality_criteria: Optional[bool] = None,
) -> str:
    """Expressão usada nos gráficos do LCR, com preservação bruta por padrão.

    Quando o usuário ativa a estratificação de qualidade, códigos sentinela e
    valores acima do teto de plausibilidade do parâmetro passam a NULL apenas
    na consulta analítica. O valor armazenado no CSV/Parquet/DuckDB não é
    sobrescrito, arredondado, truncado, winsorizado ou imputado.
    """
    bruto = f"({value_expr})"
    if not sinan_lcr_quality_criteria_enabled(apply_quality_criteria):
        return bruto
    ...
    conditions = [f"{bruto} IN ({codes_sql})"]
    if meta and meta.teto_plausivel is not None:
        conditions.append(f"{bruto} > {repr(float(meta.teto_plausivel))}")
    return f"CASE WHEN {' OR '.join(conditions)} THEN NULL ELSE {bruto} END"


def sinan_lcr_analysis_value_expr(
    value_expr: str,
    param_key: Optional[str] = None,
    *,
    enforce_percent_range: bool = False,
    apply_quality_criteria: Optional[bool] = None,
) -> str:
    clean = sinan_lcr_clean_value_expr(
        value_expr,
        param_key,
...
def sinan_lcr_numeric_audit_exprs(value_expr: str, param_key: str) -> Dict[str, str]:
    """Expressões SQL para valor bruto, valor analítico e flags de auditoria."""
    ...
    if meta and "percentual" in meta.tipo_valor:
        flag_percentual = f"CASE WHEN ({sem_sentinela}) IS NOT NULL AND (({sem_sentinela}) < 0 OR ({sem_sentinela}) > 100) THEN TRUE ELSE FALSE END"
    else:
        flag_percentual = "FALSE"
    return {
        "valor_bruto": bruto,
        "valor_limpo": valor_analise,
...
SOURCE_CONFIG: Dict[str, SourceConfig] = {
    "SINAN": SourceConfig(
        ...
        field_notes=[
            ...
            "Na área de sobreposição, NU_NOTIFIC/NM_PACIENT apenas iniciam a triagem; DT_NASC/ANO_NASC, NM_MAE_PAC, CS_SEXO válido, DT_NOTIFIC, DT_DIGITA, DT_INVEST, ATE_INTERN, DT_SIN_PRI, LAB_PUNCAO, LAB_DTPUNC, CLASSI_FIN, CON_DIAGES e o contexto administrativo determinam a classificação multicritério.",
...
FIELD_GUIDE = {
    "SINAN": [
        ...
        ("DT_DIGITA / DT_INVEST / ATE_INTERN", "cronologia da sobreposição", "primeira digitação, início da investigação e internação; DT_DIGITA exata é sinal forte do mesmo registro entre extrações"),
        ("DT_NASC / ANO_NASC + CS_SEXO", "identidade demográfica", "usa a data completa de nascimento quando disponível; ano é fallback; I/9/ignorado em sexo não conta como concordância"),
        ("NM_MAE_PAC", "identidade complementar", "reforça ou contradiz a identidade sem ser obrigatório; divergência isolada é encaminhada à revisão manual"),
        ("NU_NOTIFIC", "identificador operacional", "inicia a triagem no mesmo ano de notificação quando disponível; município/UF/unidade entram como evidência contextual"),
        ...
        ("LAB_PUNCAO", "investigação e sobreposição", "realização da punção laboratorial/lombar; mudança pode representar atualização entre extrações ou novo episódio conforme as demais evidências"),
        ("LAB_DTPUNC / DT_PUNCA / DT_PUNCAO", "data e sobreposição", "data da punção lombar; entra na janela temporal e, quando exata, reforça o mesmo registro"),
...
def literal_str_expr(col: str) -> str:
    """Preserva o conteúdo não vazio exatamente como armazenado na célula.

    Apenas NULL, string vazia e células compostas exclusivamente por espaços são
    tratadas como vazias. Espaços ao redor de um código, casas decimais e zeros à
    esquerda não são corrigidos, permitindo a auditoria fiel contra o dicionário.
    """
    raw = f"CAST({qident(col)} AS VARCHAR)"
    return f"CASE WHEN {raw} IS NULL OR TRIM({raw}) = '' THEN NULL ELSE {raw} END"
...
def dictionary_code_expr(col: str, pad2: bool = False) -> str:
    """Padroniza códigos sem transformar texto malformado em valor válido.

    Colunas numéricas de Parquet/planilha perdem zeros à esquerda por definição;
    por isso, inteiros como 1 e 7 podem representar os códigos 01 e 07. Em
    colunas textuais, somente dígitos sem espaços ou pontuação são aceitos para
    essa padronização. O conteúdo inválido é preservado para a auditoria contra
    o domínio do dicionário.
    """
    quoted = qident(col)
    raw = f"CAST({quoted} AS VARCHAR)"
    numeric_type = (
        f"regexp_matches(UPPER(typeof({quoted})), "
        "'^(UTINYINT|USMALLINT|UINTEGER|UBIGINT|TINYINT|SMALLINT|INTEGER|BIGINT|HUGEINT|UHUGEINT|FLOAT|REAL|DOUBLE|DECIMAL)')"
    )
    decimal_value = f"TRY_CAST({quoted} AS DECIMAL(38,10))"
    candidate = f"""
        CASE
            WHEN {raw} IS NULL OR TRIM({raw}) = '' THEN NULL
            WHEN {numeric_type}
                 AND {decimal_value} IS NOT NULL
                 AND {decimal_value} = TRUNC({decimal_value})
                 AND {decimal_value} >= 0
                THEN CAST(TRY_CAST({quoted} AS HUGEINT) AS VARCHAR)
            WHEN NOT ({numeric_type}) AND regexp_matches({raw}, '^[0-9]+$') THEN {raw}
            ELSE {raw}
        END
    """
    if pad2:
        return (
            f"CASE WHEN ({candidate}) IS NULL THEN NULL "
            f"WHEN regexp_matches(({candidate}), '^[0-9]$') THEN '0' || ({candidate}) "
            f"ELSE ({candidate}) END"
        )
    return candidate
...
def loaded_table_origin_format(table: LoadedTable) -> str:
    """Exibe o formato enviado, mesmo quando CSV/DBF foi convertido para Parquet."""
    label_upper = str(table.label or "").upper()
    if "CSV" in label_upper:
        return "CSV"
    if "DBF" in label_upper:
        return "DBF"
    return "DUCKDB" if table.kind.lower() == "duckdb" else "PARQUET"
...
class ColumnSelection:
    ...
    lab_ctliqu_col: Optional[str] = None
    lab_ctsang_col: Optional[str] = None
    lab_bcliqu_col: Optional[str] = None
    lab_ciliqu_col: Optional[str] = None
    lab_agliqu_col: Optional[str] = None
    lab_isliqu_col: Optional[str] = None
    lab_pcliqu_col: Optional[str] = None
    ...
    nu_notific_col: Optional[str] = None
    nm_pacient_col: Optional[str] = None
    # SIM
...
def default_selections(source: str, columns: Sequence[str]) -> ColumnSelection:
    ...
    if source == "SINAN":
        sel.nu_notific_col = choose_candidate(columns, ["NU_NOTIFIC", "NU_NOTFIC", "NUM_NOTIFIC", "NUNOTIFIC", "NU_NOTIF"])
        sel.nm_pacient_col = choose_candidate(columns, ["NM_PACIENT", "NOME_PACIENTE", "NOME", "NM_PACIENTE"])
        ...
        sel.lab_ctliqu_col = choose_candidate(columns, ["LAB_CTLIQU", "LAB_CTLIQU.", "CULTURA_LIQUOR", "CULT_LIQUOR"])
        sel.lab_ctsang_col = choose_candidate(columns, ["LAB_CTSANG", "CULTURA_SANGUE", "CULTURA_SORO", "CULT_SANGUE"])
        sel.lab_bcliqu_col = choose_candidate(columns, ["LAB_BCLIQU", "BACTERIOSCOPIA_LIQUOR", "BACT_LIQUOR"])
        sel.lab_ciliqu_col = choose_candidate(columns, ["LAB_CILIQU", "CIE_LIQUOR"])
        sel.lab_agliqu_col = choose_candidate(columns, ["LAB_AGLIQU", "LATEX_LIQUOR", "AGLUTINACAO_LATEX_LIQUOR"])
        sel.lab_isliqu_col = choose_candidate(columns, ["LAB_ISLIQU", "ISOLAMENTO_VIRAL_LIQUOR", "ISOL_VIRAL_LIQUOR"])
        sel.lab_pcliqu_col = choose_candidate(columns, ["LAB_PCLIQU", "PCR_LIQUOR"])
...
def build_expressions(source: str, sel: ColumnSelection) -> Dict[str, Optional[str]]:
    ...
    if source == "SINAN":
        exprs["nu_notific_raw"] = clean_str_expr(sel.nu_notific_col) if sel.nu_notific_col else None
        exprs["nm_pacient_raw"] = clean_str_expr(sel.nm_pacient_col) if sel.nm_pacient_col else None
        exprs["classi_raw"] = literal_str_expr(sel.classi_fin_col) if sel.classi_fin_col else None
        exprs["classi_code"] = dictionary_code_expr(sel.classi_fin_col) if sel.classi_fin_col else None
        ...
        exprs["con_raw"] = literal_str_expr(sel.con_diages_col) if sel.con_diages_col else None
        exprs["con_code"] = dictionary_code_expr(sel.con_diages_col, pad2=True) if sel.con_diages_col else None
        ...
        exprs["cla_me_bac_raw"] = literal_str_expr(sel.cla_me_bac_col) if sel.cla_me_bac_col else None
        exprs["cla_me_bac_code"] = dictionary_code_expr(sel.cla_me_bac_col, pad2=True) if sel.cla_me_bac_col else None
        ...
        exprs["cla_me_ass_raw"] = literal_str_expr(sel.cla_me_ass_col) if sel.cla_me_ass_col else None
        exprs["cla_me_ass_code"] = dictionary_code_expr(sel.cla_me_ass_col, pad2=True) if sel.cla_me_ass_col else None
        ...
        exprs["criterio_code"] = dictionary_code_expr(sel.criterio_col, pad2=True) if sel.criterio_col else None
        exprs["criterio_label"] = case_from_mapping(exprs["criterio_code"], SINAN_CRITERIO, "Sem preenchimento / preenchimento errado") if exprs["criterio_code"] else None
        ...
        exprs["puncao_raw"] = literal_str_expr(sel.lab_puncao_col) if sel.lab_puncao_col else None
        ...
        exprs["lab_aspect_raw"] = literal_str_expr(sel.lab_aspect_col) if sel.lab_aspect_col else None
        ...
        for param_key, param_col in [
            ("hema", sel.lab_hema_col),
            ("neutro", sel.lab_neutro_col),
            ("glico", sel.lab_glico_col),
            ("leuco", sel.lab_leuco_col),
            ("eosi", sel.lab_eosi_col),
            ("prot", sel.lab_prot_col),
            ("mono", sel.lab_mono_col),
            ("linfo", sel.lab_linfo_col),
            ("clor", sel.lab_clor_col),
        ]:
            exprs[f"lab_{param_key}_raw"] = literal_str_expr(param_col) if param_col else None
            exprs[f"lab_{param_key}"] = numeric_expr(param_col) if param_col else None
        for method_key, method_col in [
            ("lab_ctliqu", sel.lab_ctliqu_col),
            ("lab_ctsang", sel.lab_ctsang_col),
            ("lab_bcliqu", sel.lab_bcliqu_col),
            ("lab_ciliqu", sel.lab_ciliqu_col),
            ("lab_agliqu", sel.lab_agliqu_col),
            ("lab_isliqu", sel.lab_isliqu_col),
            ("lab_pcliqu", sel.lab_pcliqu_col),
        ]:
            exprs[f"{method_key}_raw"] = literal_str_expr(method_col) if method_col else None
            exprs[f"{method_key}_code"] = dictionary_code_expr(method_col, pad2=True) if method_col else None
...
def query_sinan_criterion_presence_by_classification(
    table: LoadedTable,
    classi_sql: str,
    criterion_sql: str,
    where_sql: str,
) -> pd.DataFrame:
    """Compara presença de CRITERIO em confirmados, descartados e no total válido."""
    ...
    sql = f"""
        WITH casos_validos AS (
            ...
            SELECT *
            FROM casos_validos
            WHERE classificacao IN ('1', '2')
        ), estratos AS (
            SELECT 'Total de casos' AS estrato, presenca FROM classificados
            UNION ALL
            SELECT CASE classificacao
                       WHEN '1' THEN 'Casos confirmados'
                       WHEN '2' THEN 'Casos descartados'
                   END AS estrato,
                   presenca
            FROM classificados
        ), agregado AS (
            SELECT estrato, presenca, COUNT(*) AS n
            FROM estratos
            GROUP BY 1, 2
        ), totais AS (
            SELECT estrato, SUM(n) AS denominador
            FROM agregado
            GROUP BY 1
        ...
        SELECT e.estrato,
               p.presenca,
               COALESCE(a.n, 0) AS n,
               COALESCE(t.denominador, 0) AS denominador,
               CASE WHEN COALESCE(t.denominador, 0) > 0
                    THEN ROUND(100.0 * COALESCE(a.n, 0) / t.denominador, 2)
                    ELSE NULL END AS pct,
               e.ordem_estrato,
               p.ordem_presenca
        FROM ordem_estratos e
        CROSS JOIN ordem_presencas p
        LEFT JOIN agregado a USING (estrato, presenca)
        LEFT JOIN totais t USING (estrato)
...
def query_sinan_criterion_timeseries(
    ...
    allowed_codes = [
        ...
        if criterion_codes is None or code in {str(item).zfill(2) for item in criterion_codes}
    ]
    if not allowed_codes:
        return pd.DataFrame()
    criterion_rows_sql = ",\n                ".join(
        f"({qstr(code)}, {qstr(SINAN_CRITERIO[code])}, {order})"
        for order, code in enumerate(allowed_codes, start=1)
    )
    valid_codes_sql = ", ".join(qstr(code) for code in allowed_codes)
    sql = f"""
        WITH criterios(codigo, criterio, ordem) AS (
            VALUES {criterion_rows_sql}
        ), universo AS (
...
def render_sinan_criterion_distribution_chart(
    ...
    criterion_code = exprs.get("criterio_code")
    if not classi_code or not criterion_code:
        st.warning(
    ...
    st.caption(
        f"Denominador: {denominator_text} caso(s) em {stratum.lower()}, considerando apenas CLASSI_FIN válido (1 ou 2); "
...
def render_sinan_criterion_timeseries_chart(
    ...
    heading_suffix = f" — {heading_context}" if heading_context else ""
    st.markdown(f"#### Critérios diagnósticos ao longo do tempo{heading_suffix}")
    st.caption(
        "Como ler: cada cor representa um código oficial de `CRITERIO` do Quadro V. Os pontos são os resultados "
    ...
    missing_fields = [
        ...
        for label, expression in (
            ...
            ("CLASSI_FIN", classi_code),
            ("CRITERIO", criterion_code),
        )
        if not expression
    ]
    if missing_fields:
    ...
    if plotted.empty:
        ...
        fig = go.Figure()
        allowed_code_set = (
            {str(code).zfill(2) for code in criterion_codes}
            if criterion_codes is not None
            else set(SINAN_CRITERIO)
        )
        criterion_order = [
        ...
        fig.update_layout(
            title=f"Critérios diagnósticos ao longo do tempo — {heading_context or stratum.lower()}",
            xaxis_title="Ano",
            yaxis_title=y_label,
    ...
    if population_note:
        denominator_note = (
            "O denominador de cada ano contém somente os casos com data válida que atendem ao recorte etiológico "
            f"informado: {population_note}."
        )
    else:
        denominator_note = (
    ...
    st.caption(
        denominator_note
        + " Metodologia da suavização: para estimar a tendência em cada posição do eixo temporal, o LOESS ajusta "
        "uma regressão linear local usando os 2/3 dos anos mais próximos e atribui maior peso aos anos mais próximos "
        "por uma função tricúbica. Não foi aplicada etapa de reponderação robusta. A curva não extrapola além do "
        "primeiro e do último ano observados, não altera os pontos anuais nem deve ser interpretada como previsão, "
    ...
    download_button(
        display_df,
        f"sinan_criterios_tempo_{safe_filename(heading_context or stratum)}.csv",
    )
...
def query_age_frequency(
    table: LoadedTable,
    age_sql: str,
    where_sql: str,
    group_sql: Optional[str] = None,
) -> pd.DataFrame:
    """Frequência por idade exata para densidade ponderada sem expandir a base."""
    group_expr = group_sql or qstr("Casos/registros analisados")
    sql = f"""
        WITH base AS (
            SELECT {age_sql} AS idade, {group_expr} AS grupo
            FROM {table.ref_sql}
            {where_sql}
        )
        SELECT idade, grupo, COUNT(*) AS n
        FROM base
        WHERE idade BETWEEN 0 AND 130 AND grupo IS NOT NULL
        GROUP BY 1, 2
        ORDER BY grupo, idade
    """
    return run_query(table, sql)
...
def _weighted_quantile(values: np.ndarray, weights: np.ndarray, probability: float) -> float:
    ...
    if cumulative.size == 0 or cumulative[-1] <= 0:
        return float("nan")
    target = float(np.clip(probability, 0.0, 1.0)) * cumulative[-1]
...
def _weighted_density(
    ...
    *,
    ...
    if iqr > 0:
        ...
        grid_min = max(fence_low, float(values.min())) - 3.0 * bandwidth
        grid_max = min(fence_high, float(values.max())) + 3.0 * bandwidth
    else:
        # Sem dispersão interquartil estimável, mantém-se o fallback visual anterior.
        grid_min = _weighted_quantile(values, weights, 0.005) - 3.0 * bandwidth
        grid_max = _weighted_quantile(values, weights, 0.995) + 3.0 * bandwidth
    ...
    *,
    ...
    for group_index, group in enumerate(groups_present):
        ...
        fig.add_trace(go.Scatter(
            ...
            y=np.concatenate([baseline + density_scaled, np.full_like(grid, baseline)[::-1]]),
            mode="lines",
    ...
    fig.update_layout(
        ...
        yaxis={
            ...
            "range": [-0.42, max(len(groups_present) - 1, 0) + 0.48],
            "title": "Grupo",
            "zeroline": False,
        },
        height=max(700, 420 * max(1, len(groups_present))),
        showlegend=False,
        hovermode="closest",
    )
...
def query_sinan_lcr_numeric_audit_long(
    ...
    """Lista registros atingidos por sentinela ou teto de plausibilidade.

    ``linha_fonte`` é calculada antes dos filtros e identifica a posição do
    registro na relação lida pelo DuckDB nesta execução. Identificadores do caso
    são incluídos quando existem, sem substituir a linha de origem.
    """
    params = sinan_quimio_param_exprs(exprs)
    if not params:
        return pd.DataFrame()
    unions = []
    nu_notific = exprs.get("nu_notific_raw") or "CAST(NULL AS VARCHAR)"
    nm_pacient = exprs.get("nm_pacient_raw") or "CAST(NULL AS VARCHAR)"
    classi_code = exprs.get("classi_code") or "CAST(NULL AS VARCHAR)"
    for ordem, (key, label, value_expr) in enumerate(params, start=1):
        meta = sinan_lcr_param_metadata(key)
        audit = sinan_lcr_numeric_audit_exprs(value_expr, key)
        teto_sql = "CAST(NULL AS DOUBLE)" if not meta or meta.teto_plausivel is None else repr(float(meta.teto_plausivel))
        unions.append(
            f"""
            SELECT {int(ordem)} AS ordem_parametro,
                   __linha_fonte AS linha_fonte,
                   {qstr(loaded_table_origin_format(table))} AS formato_origem,
                   {qstr(table.label or table.source)} AS origem,
                   {nu_notific} AS NU_NOTIFIC,
                   {nm_pacient} AS NM_PACIENT,
                   {classi_code} AS CLASSI_FIN,
    ...
    limit_sql = "" if limit is None else f"LIMIT {int(max(1, limit))}"
    sql = f"""
        WITH fonte AS (
            ...
            SELECT *,
                   CASE
                       WHEN flag_sentinela THEN 'Código sentinela'
                       WHEN flag_acima_teto_plausivel THEN 'Acima do teto de plausibilidade'
                       ELSE NULL
                   ...
                   CASE
                       ...
                       ELSE NULL
                   END AS efeito_na_analise
            FROM long
        )
        SELECT *
        FROM afetados
        WHERE flag_sentinela OR flag_acima_teto_plausivel
        ORDER BY linha_fonte, ordem_parametro
        {limit_sql}
    """
    return run_query(table, sql)
...
def query_sinan_numeric_distribution_stratified_by_reference_bins(
    ...
    # O helper preserva o bruto por padrão. Apenas sob opt-in ele neutraliza
    # sentinelas e valores acima do teto antes das faixas clínicas.
...
def _lcr_predominio_expr(exprs: Dict[str, Optional[str]]) -> Optional[str]:
    ...
    Usa o wrapper de LCR sem alterar o bruto por padrão. Sob opt-in, neutraliza
    sentinelas e percentuais fora de 0-100 para o cálculo de predomínio.
    """
...
def query_sinan_discarded_meningitis_risk(
    ...
    def _flag_query(label: str, param_key: str, value_expr: Optional[str], threshold_sql: str) -> Optional[pd.DataFrame]:
        if not value_expr:
            return None
        valor_limpo = sinan_lcr_clean_value_expr(value_expr, param_key)
        sql = f"""
            WITH base AS (
    ...
    pleo = _flag_query("Pleocitose isolada (leucócitos ≥ 20 céls/mm³)", "leuco", leuco, "valor >= 20")
    if pleo is not None and not pleo.empty:
        rows.append(pleo)
    gli = _flag_query("Glicose reduzida isolada (< 40 mg/dL no LCR)", "glico", glico, "valor < 40")
    if gli is not None and not gli.empty:
        rows.append(gli)
    pr = _flag_query("Proteína elevada isolada (≥ 45 mg/dL)", "prot", prot, "valor >= 45")
...
def render_lcr_quality_mode_note() -> None:
...
def render_sinan_lcr_indicators(table: LoadedTable, exprs: Dict[str, Optional[str]], base_where: str, graph_where: str) -> None:
    ...
    apply_quality_criteria = st.toggle(
        "Estratificar a análise aplicando códigos sentinela e tetos de plausibilidade",
        value=False,
        key=SINAN_LCR_QUALITY_TOGGLE_KEY,
        help=(
            "Desativado (padrão): mantém sentinelas e valores extremos nas estatísticas e gráficos. "
            "Ativado: trata sentinelas e valores acima do teto como ausentes somente na visão analítica, "
            "sem modificar o arquivo-fonte, e exibe a tabela completa dos registros afetados."
        ),
    )
    if apply_quality_criteria:
        st.warning(
            "Modo com critérios ativado: códigos 999/9999/99999 e valores acima do teto de plausibilidade "
            "do respectivo parâmetro não entram nos cálculos do LCR. Os valores originais continuam intactos "
            "e aparecem na tabela de rastreabilidade abaixo."
        )
    else:
        st.info(
            "Modo de fidelidade bruta (padrão): códigos sentinela e valores acima dos tetos permanecem nos "
            "gráficos e estatísticas. O painel apenas interpreta o conteúdo como número para posicioná-lo no "
            "gráfico; não corrige, substitui, trunca, arredonda, winsoriza nem imputa o valor armazenado."
        )
    ...
    if apply_quality_criteria:
        st.markdown("**Registros atingidos pelos critérios ativados**")
        audit_df = query_sinan_lcr_numeric_audit_long(table, exprs, graph_where, limit=None)
        if audit_df.empty:
            st.success("Nenhum registro do recorte atual corresponde a código sentinela ou ultrapassa teto de plausibilidade.")
        else:
            st.caption(
                "Cada linha identifica o registro na relação lida nesta execução, o campo afetado, o valor bruto e o motivo. "
                "Em arquivos compostos por vários Parquets ou após reordenação física do banco, use também NU_NOTIFIC/NM_PACIENT "
                "para localizar o caso, pois a numeração da linha pode mudar entre leituras."
            )
            audit_display = audit_df.drop(columns=["ordem_parametro"], errors="ignore")
            copyable_dataframe(audit_display, width="stretch", hide_index=True)
            download_button(
                audit_display,
                "sinan_lcr_registros_afetados_sentinelas_tetos.csv",
                label="Baixar todos os registros afetados (CSV)",
                max_rows=len(audit_display),
            )
...
def query_sinan_overlap_summary(
    ...
    target_col: str,
    where_sql: str,
    target_expr_override: Optional[str] = None,
) -> pd.DataFrame:
    """Resumo de sobreposição para uma coluna genérica.

    ``target_expr_override`` permite que NM_PACIENT seja contado com a mesma
    normalização usada na comparação multicritério, sem alterar o valor bruto.
    """
    target_expr = target_expr_override or clean_str_expr(target_col)
    sql = f"""
        WITH base AS (
...
# =============================================================================
# Sobreposição SINAN — classificação multicritério por pares
# =============================================================================
# NU_NOTIFIC e NM_PACIENT funcionam como chaves de triagem. A classificação final
# compara identidade, cronologia, contexto administrativo, investigação e punção
# entre cada par. Nenhuma classificação exclui registros automaticamente.

SINAN_OVERLAP_DATE_FIELDS: Tuple[Tuple[str, str], ...] = (
    ("DT_NOTIFIC", "data_notificacao"),
    ("DT_DIGITA", "data_digitacao"),
    ("DT_INVEST", "data_investigacao"),
    ("ATE_INTERN", "data_internacao"),
    ("DT_SIN_PRI", "data_primeiros_sintomas"),
    ("LAB_DTPUNC", "data_puncao"),
)
DEFAULT_SINAN_OVERLAP_CLOSE_DAYS = 7
DEFAULT_SINAN_OVERLAP_DISTINCT_DAYS = 90
DEFAULT_SINAN_OVERLAP_MIN_DATES = 1
DEFAULT_SINAN_OVERLAP_MAX_PAIRS = 50000
DEFAULT_SINAN_OVERLAP_MAX_PAIRS_PER_KEY = 5000
DEFAULT_SINAN_OVERLAP_FUZZY_THRESHOLD = 0.93
DEFAULT_SINAN_OVERLAP_FUZZY_MAX_RECORDS = 50000
OVERLAP_NEAR_NAME_THRESHOLD = 0.94


def _sinan_overlap_context_date_expr(
    context_cols: Dict[str, Optional[str]],
    key: str,
    fallback: Optional[str] = None,
) -> str:
    col = context_cols.get(key)
    if col:
        return date_expr(col)
    return fallback or "NULL"


def _sinan_overlap_notification_year_expr(
    context_cols: Dict[str, Optional[str]],
    exprs: Optional[Dict[str, Optional[str]]] = None,
) -> str:
    """Obtém o ano administrativo da notificação, priorizando NU_ANO e derivando de DT_NOTIFIC."""
    year_col = context_cols.get("notification_year")
    if year_col:
        code = clean_code_expr(year_col)
        parsed = f"TRY_CAST(({code}) AS INTEGER)"
        return f"CASE WHEN {parsed} BETWEEN 1900 AND 2100 THEN {parsed} ELSE NULL END"
    fallback = (exprs or {}).get("dt_notificacao")
    dt_sql = _sinan_overlap_context_date_expr(context_cols, "date_notification", fallback)
    if dt_sql != "NULL":
        return f"CAST(EXTRACT(YEAR FROM ({dt_sql})) AS INTEGER)"
    return "NULL"


def _sinan_overlap_group_expr(
    target_col: str,
    candidate_type: str,
    context_cols: Optional[Dict[str, Optional[str]]] = None,
    exprs: Optional[Dict[str, Optional[str]]] = None,
) -> str:
    """Monta a chave candidata sem presumir unicidade estadual de NU_NOTIFIC.

    Para nomes, usa comparação exata após normalização de acentos, caixa e
    pontuação. Para notificações, combina NU_NOTIFIC com o ano da notificação
    quando esse ano pode ser obtido. UF, município e unidade notificadora ficam
    como evidências par a par, evitando perder atualizações em que algum desses
    campos esteja temporariamente ausente.
    """
    if candidate_type == "name":
        return _normalized_identity_expr(target_col)
    raw = clean_str_expr(target_col)
    year_sql = _sinan_overlap_notification_year_expr(context_cols or {}, exprs)
    if year_sql == "NULL":
        return raw
    return f"""
        CASE
            WHEN {raw} IS NULL THEN NULL
            ELSE CAST({raw} AS VARCHAR) || '|ANO=' || COALESCE(CAST(({year_sql}) AS VARCHAR), 'SEM_ANO')
        END
    """


def _sinan_overlap_birth_year_expr(context_cols: Dict[str, Optional[str]]) -> str:
    """Usa ANO_NASC quando presente; deriva o ano de DT_NASC como fallback."""
    birth_year_col = context_cols.get("birth_year")
    if birth_year_col:
        code = clean_code_expr(birth_year_col)
        parsed = f"TRY_CAST(({code}) AS INTEGER)"
        return f"CASE WHEN {parsed} BETWEEN 1800 AND 2100 THEN {parsed} ELSE NULL END"
    birth_date_col = context_cols.get("birth_date")
    if birth_date_col:
        birth_date_sql = date_expr(birth_date_col)
        return f"CAST(EXTRACT(YEAR FROM ({birth_date_sql})) AS INTEGER)"
    return "NULL"


def _sinan_overlap_date_sql_map(
    context_cols: Dict[str, Optional[str]],
    exprs: Dict[str, Optional[str]],
) -> Dict[str, str]:
    return {
        "DT_NOTIFIC": _sinan_overlap_context_date_expr(
            context_cols, "date_notification", exprs.get("dt_notificacao")
        ),
        "DT_DIGITA": _sinan_overlap_context_date_expr(context_cols, "date_entry"),
        "DT_INVEST": _sinan_overlap_context_date_expr(context_cols, "date_investigation"),
        "ATE_INTERN": _sinan_overlap_context_date_expr(context_cols, "date_hospitalization"),
        "DT_SIN_PRI": _sinan_overlap_context_date_expr(
            context_cols, "date_symptom", exprs.get("dt_sin_pri")
        ),
        "LAB_DTPUNC": _sinan_overlap_context_date_expr(
            context_cols, "date_puncture", exprs.get("dt_puncao")
        ),
    }


def query_sinan_overlap_date_coverage(
    table: LoadedTable,
    where_sql: str,
    exprs: Dict[str, Optional[str]],
    context_cols: Dict[str, Optional[str]],
) -> pd.DataFrame:
    """Mede quantos campos temporais realmente têm algum valor no recorte."""
    date_sql = _sinan_overlap_date_sql_map(context_cols, exprs)
    aliases = {label: f"n_{idx}" for idx, (label, _column) in enumerate(SINAN_OVERLAP_DATE_FIELDS, start=1)}
    select_parts = ["COUNT(*) AS n_total"]
    for label, _column in SINAN_OVERLAP_DATE_FIELDS:
        expr = date_sql.get(label, "NULL")
        select_parts.append(f"COUNT(*) FILTER (WHERE ({expr}) IS NOT NULL) AS {aliases[label]}")
    sql = f"SELECT {', '.join(select_parts)} FROM {table.ref_sql} {where_sql}"
    ...
    key_by_label = {
        ...
        "DT_DIGITA": "date_entry",
        "DT_INVEST": "date_investigation",
        "ATE_INTERN": "date_hospitalization",
        "DT_SIN_PRI": "date_symptom",
        "LAB_DTPUNC": "date_puncture",
    }
    rows: List[Dict[str, object]] = []
    for label, _column in SINAN_OVERLAP_DATE_FIELDS:
        n = int(row.get(aliases[label], 0) or 0)
        rows.append(
            {
                "Campo temporal": label,
                "Coluna detectada": context_cols.get(key_by_label[label]) or "—",
                "Registros preenchidos": n,
                "Cobertura (%)": round(100.0 * n / total, 2) if total > 0 else np.nan,
                "Disponível no recorte": "Sim" if n > 0 else "Não",
            }
        )
    return pd.DataFrame(rows)


def query_sinan_overlap_candidate_records(
    table: LoadedTable,
    target_col: str,
    where_sql: str,
    exprs: Dict[str, Optional[str]],
    ...
    *,
    candidate_type: str,
    only_repeated_keys: bool = True,
    limit: Optional[int] = None,
) -> pd.DataFrame:
    """Retorna registros candidatos com identidade, datas, contexto e punção.

    Para NU_NOTIFIC, a chave candidata é escopada pelo ano da notificação quando
    disponível. Para nomes, a chave exata é normalizada; a busca fuzzy opcional
    usa esta mesma extração com ``only_repeated_keys=False``.
    """
    target_raw_sql = clean_str_expr(target_col)
    target_group_sql = _sinan_overlap_group_expr(
        target_col, candidate_type, context_cols=context_cols, exprs=exprs
    )

    nu_col = context_cols.get("nu_notific")
    patient_col = context_cols.get("patient")
    mother_col = context_cols.get("mother_name")
    birth_date_col = context_cols.get("birth_date")
    sex_col = context_cols.get("sex")
    agravo_col = context_cols.get("agravo")
    uf_col = context_cols.get("uf_notification")
    municipality_col = context_cols.get("municipality_notification")
    unit_col = context_cols.get("notification_unit")
    municipality_res_col = context_cols.get("municipality_residence")
    classi_col = context_cols.get("classification_final")
    con_col = context_cols.get("diagnosis_conclusion")
    criterio_col = context_cols.get("confirmation_criterion")
    evol_col = context_cols.get("outcome")
    puncao_col = context_cols.get("lab_puncture")

    nu_sql = clean_str_expr(nu_col) if nu_col else "NULL"
    ...
    birth_date_sql = date_expr(birth_date_col) if birth_date_col else "NULL"
    birth_year_sql = _sinan_overlap_birth_year_expr(context_cols)
    notification_year_sql = _sinan_overlap_notification_year_expr(context_cols, exprs)
    sex_sql = clean_code_expr(sex_col) if sex_col else "NULL"
    agravo_sql = clean_str_expr(agravo_col) if agravo_col else "NULL"
    uf_sql = clean_str_expr(uf_col) if uf_col else "NULL"
    ...
    puncao_code_sql = clean_code_expr(puncao_col) if puncao_col else "NULL"
    puncao_label_sql = (
        case_from_mapping(puncao_code_sql, YES_NO_IGN, "Sem informação") if puncao_col else "NULL"
    )

    date_sql = _sinan_overlap_date_sql_map(context_cols, exprs)
    dt_notification_sql = date_sql["DT_NOTIFIC"]
    dt_entry_sql = date_sql["DT_DIGITA"]
    dt_investigation_sql = date_sql["DT_INVEST"]
    dt_hospitalization_sql = date_sql["ATE_INTERN"]
    dt_symptom_sql = date_sql["DT_SIN_PRI"]
    dt_puncture_sql = date_sql["LAB_DTPUNC"]
    dt_closure_sql = _sinan_overlap_context_date_expr(
        context_cols, "date_closure", exprs.get("dt_encerramento")
    )
    ...
    criterio_label_sql = exprs.get("criterio_label") or criterio_code_sql
    evol_label_sql = exprs.get("evol_label") or evol_code_sql

    chronology_sql = _sinan_chronology_alert_expr(
        dt_symptom_sql if dt_symptom_sql != "NULL" else None,
        dt_notification_sql if dt_notification_sql != "NULL" else None,
        dt_investigation_sql if dt_investigation_sql != "NULL" else None,
        dt_closure_sql if dt_closure_sql != "NULL" else None,
    )
    repeated_filter = "registros_mesma_chave_candidata > 1" if only_repeated_keys else "TRUE"
    limit_sql = "" if limit is None else f" LIMIT {max(1, int(limit))}"

    sql = f"""
        WITH base AS (
            SELECT
                ROW_NUMBER() OVER () AS id_registro_fonte,
                {target_raw_sql} AS valor_candidato,
                {target_group_sql} AS chave_candidata,
                {nu_sql} AS nu_notific,
                {patient_sql} AS nm_pacient,
                {mother_sql} AS nm_mae_pac,
                {birth_date_sql} AS data_nascimento,
                {birth_year_sql} AS ano_nascimento,
                {sex_sql} AS sexo,
                {notification_year_sql} AS ano_notificacao,
                {dt_notification_sql} AS data_notificacao,
                {dt_entry_sql} AS data_digitacao,
                {dt_investigation_sql} AS data_investigacao,
                {dt_hospitalization_sql} AS data_internacao,
                {dt_symptom_sql} AS data_primeiros_sintomas,
                {dt_puncture_sql} AS data_puncao,
                {dt_closure_sql} AS data_encerramento,
                {puncao_code_sql} AS lab_puncao_codigo,
                {puncao_label_sql} AS lab_puncao,
                {classi_code_sql} AS classi_fin_codigo,
                {classi_label_sql} AS classificacao_final,
                {con_code_sql} AS con_diages_codigo,
                {con_label_sql} AS conclusao_diagnostica,
                {con_group_sql} AS grupo_etiologico,
                {criterio_code_sql} AS criterio_codigo,
                {criterio_label_sql} AS criterio_confirmacao,
                {evol_code_sql} AS evolucao_codigo,
                {evol_label_sql} AS evolucao,
                {agravo_sql} AS id_agravo,
                {uf_sql} AS uf_notificacao,
            ...
            SELECT *
            FROM base
            WHERE chave_candidata IS NOT NULL
        ), marcados AS (
            SELECT
                *,
                COUNT(*) OVER (PARTITION BY chave_candidata) AS registros_mesma_chave_candidata
            FROM valid
        )
        SELECT
            ROW_NUMBER() OVER (
                ORDER BY chave_candidata, data_primeiros_sintomas NULLS LAST,
                         data_notificacao NULLS LAST, data_digitacao NULLS LAST,
                         data_puncao NULLS LAST, id_registro_fonte
            ) AS ordem_exportacao,
            *
        FROM marcados
        WHERE {repeated_filter}
        ORDER BY ordem_exportacao{limit_sql}
    """
    return run_query(table, sql, cache=False)


def _overlap_scalar(value: object) -> Optional[object]:
    """Converte ausências heterogêneas do pandas/SQL em ``None``."""
    if value is None:
        return None
    try:
    ...
    if isinstance(value, str):
        cleaned = value.strip()
        if not cleaned or cleaned.upper() in {"NA", "N/A", "NAN", "NONE", "NULL", "<NA>"}:
            return None
        return cleaned
    return value


def _overlap_text(value: object) -> Optional[str]:
    scalar = _overlap_scalar(value)
    if scalar is None:
        return None
    return str(scalar).strip()


def _overlap_name(value: object) -> Optional[str]:
    text = _overlap_text(value)
    if text is None:
        return None
...
def _overlap_name_tokens(value: object) -> Tuple[str, ...]:
    text = _overlap_text(value)
    if text is None:
        return tuple()
    folded = unicodedata.normalize("NFKD", text)
    folded = "".join(ch for ch in folded if not unicodedata.combining(ch)).upper()
    return tuple(token for token in re.findall(r"[0-9A-Z]+", folded) if token)


def _overlap_code(value: object, *, pad2: bool = False) -> Optional[str]:
    text = _overlap_text(value)
    if text is None:
        return None
    ...
    if not text:
        return None
    if pad2 and len(text) == 1:
        text = "0" + text
    return text


def _overlap_year(value: object) -> Optional[int]:
    scalar = _overlap_scalar(value)
    if scalar is None:
        return None
    try:
        year = int(float(str(scalar).replace(",", ".")))
    except Exception:
        return None
    return year if 1800 <= year <= 2100 else None


def _overlap_date(value: object) -> Optional[pd.Timestamp]:
    scalar = _overlap_scalar(value)
    if scalar is None:
        return None
    try:
        if isinstance(scalar, str) and re.fullmatch(r"\d{4}-\d{2}-\d{2}", scalar.strip()):
            parsed = pd.to_datetime(scalar, format="%Y-%m-%d", errors="coerce")
        elif isinstance(scalar, str) and re.fullmatch(r"\d{8}", scalar.strip()):
            text = scalar.strip()
            fmt = "%Y%m%d" if 1900 <= int(text[:4]) <= 2099 else "%d%m%Y"
            parsed = pd.to_datetime(text, format=fmt, errors="coerce")
        else:
            parsed = pd.to_datetime(scalar, errors="coerce", dayfirst=True)
    except Exception:
        return None
    if pd.isna(parsed):
        return None
...
def _overlap_sex(value: object) -> Optional[str]:
    """Normaliza sexo e converte I/9/ignorado em ausência real."""
    text = _overlap_text(value)
    if text is None:
        return None
    norm = unicodedata.normalize("NFKD", text)
    norm = "".join(ch for ch in norm if not unicodedata.combining(ch)).upper().strip()
    code = re.sub(r"[^0-9A-Z]", "", norm)
    if code in {"M", "1", "MASCULINO", "MASC"}:
        return "M"
    if code in {"F", "2", "3", "FEMININO", "FEM"}:
        return "F"
    if code in {"", "I", "0", "9", "IGNORADO", "IGNORADA", "IGN", "NAOINFORMADO", "SEMINFO"}:
        return None
    return None


def _overlap_yes_no(value: object) -> Optional[str]:
    text = _overlap_text(value)
    if text is None:
        return None
    norm = unicodedata.normalize("NFKD", text)
    norm = "".join(ch for ch in norm if not unicodedata.combining(ch)).upper().strip()
    code = re.sub(r"[^0-9A-Z]", "", norm)
    if code in {"1", "S", "SIM", "Y", "YES"}:
        return "SIM"
    if code in {"2", "N", "NAO", "NO"}:
        return "NAO"
    if code in {"", "0", "9", "I", "IGNORADO", "IGNORADA", "IGN", "SEMINFO"}:
        return None
    return None


def _overlap_compare(left: object, right: object, normalizer) -> Optional[bool]:
...
def _overlap_change_kind(left: object, right: object, normalizer) -> str:
    left_norm = normalizer(left)
    right_norm = normalizer(right)
    if left_norm is None and right_norm is None:
        return "sem_informacao"
    if left_norm is None or right_norm is None:
        return "preenchimento_entre_extracoes"
    if left_norm == right_norm:
        return "igual"
    return "diferente"


def _overlap_change_label(kind: str) -> str:
    return {
        "sem_informacao": "Não comparável",
        "preenchimento_entre_extracoes": "Preenchido/alterado entre extrações",
        "igual": "Igual",
        "diferente": "Diferente",
    }.get(kind, "Não comparável")


def _overlap_status_label(status: Optional[bool], *, normalized_name: bool = False, near: bool = False) -> str:
    if status is True:
        if near:
            return "Compatível por similaridade"
        return "Igual após normalização" if normalized_name else "Igual"
    if status is False:
        return "Diferente"
...
def _jaro_winkler_similarity(left: str, right: str, prefix_scale: float = 0.1) -> float:
...
def _overlap_name_similarity(left: object, right: object) -> Optional[float]:
    left_compact = _overlap_name(left)
    right_compact = _overlap_name(right)
    if not left_compact or not right_compact:
        return None
    direct = _jaro_winkler_similarity(left_compact, right_compact)
    left_sorted = "".join(sorted(_overlap_name_tokens(left)))
    right_sorted = "".join(sorted(_overlap_name_tokens(right)))
    reordered = _jaro_winkler_similarity(left_sorted, right_sorted) if left_sorted and right_sorted else 0.0
    return round(max(direct, reordered), 4)


def _overlap_effective_name_status(left: object, right: object) -> Tuple[Optional[bool], Optional[float], bool]:
    exact = _overlap_compare(left, right, _overlap_name)
    similarity = _overlap_name_similarity(left, right)
    near = bool(exact is False and similarity is not None and similarity >= OVERLAP_NEAR_NAME_THRESHOLD)
    return (True if near else exact), similarity, near


def _overlap_effective_birth_status(
    left: Dict[str, object], right: Dict[str, object]
) -> Tuple[Optional[bool], str, Optional[bool], Optional[bool]]:
    full_status = _overlap_compare(left.get("data_nascimento"), right.get("data_nascimento"), _overlap_date)
    year_status = _overlap_compare(left.get("ano_nascimento"), right.get("ano_nascimento"), _overlap_year)
    if full_status is not None:
        return full_status, "DT_NASC", full_status, year_status
    return year_status, "ANO_NASC", full_status, year_status


def _overlap_date_gaps(left: Dict[str, object], right: Dict[str, object]) -> Dict[str, Optional[int]]:
    gaps: Dict[str, Optional[int]] = {}
    for field_label, column_name in SINAN_OVERLAP_DATE_FIELDS:
        left_date = _overlap_date(left.get(column_name))
        right_date = _overlap_date(right.get(column_name))
        gaps[field_label] = (
            abs(int((right_date - left_date).days))
...
def _overlap_pair_definition(label: str) -> str:
    definitions = {
        "Duplicidade provável": (
            "Identidade compatível e cronologia curta, com número suficiente de datas ou sinal exato forte do mesmo registro."
        ),
        "Provável mesmo registro — classificação atualizada entre extrações": (
            "NU_NOTIFIC e identidade são compatíveis; DT_DIGITA/LAB_DTPUNC exata ou cronologia curta indica o mesmo registro, "
            "enquanto CLASSI_FIN/CON_DIAGES foi preenchido ou refinado entre extrações."
        ),
        "Provável mesmo registro — investigação/punção atualizada entre extrações": (
            "Identidade e cronologia sustentam o mesmo registro, mas LAB_PUNCAO foi preenchido ou alterado entre extrações."
        ),
        "Reuso de numeração provável — outro paciente": (
            "NU_NOTIFIC reaparece no mesmo escopo anual, mas há conflito em identidade estável."
        ),
        "Reuso de numeração provável — outro atendimento": (
            "NU_NOTIFIC reaparece com cronologia distante ou combinação forte de contexto administrativo e investigação incompatíveis."
        ),
        "Repetição de nome provável — homônimo/outro paciente": (
            "O nome se repete, mas DT_NASC/ANO_NASC ou CS_SEXO diverge entre os registros."
        ),
        "Repetição de nome provável — outro atendimento": (
            "O nome se repete, mas a cronologia ou o escopo administrativo indica episódio distinto."
        ),
        "Classificação divergente — revisão manual": (
            "CLASSI_FIN/CON_DIAGES diverge, porém a evidência disponível não basta para decidir entre atualização e novo atendimento."
        ),
        "Investigação/punção divergente — revisão manual": (
            "LAB_PUNCAO diverge sem ancoragem temporal suficiente para decidir se houve atualização ou novo episódio."
        ),
        "Nome quase igual — revisão manual": (
            "Par opcional encontrado por similaridade Jaro-Winkler dentro de bloco demográfico; nunca é promovido automaticamente a duplicidade."
        ),
        "Possível duplicidade — revisão manual": (
            "As evidências não sustentam nem afastam suficientemente a duplicidade; revisar os registros originais."
        ),
        "Indeterminado — dados insuficientes": (
            "Não há preenchimento suficiente dos critérios de identidade e/ou das datas comparáveis."
        ),
    }
    return definitions.get(label, "Classificação multicritério para triagem e revisão manual.")


def classify_sinan_overlap_pairs(
    records: pd.DataFrame,
    *,
    candidate_type: str,
    close_days: int,
    distinct_days: int,
    min_comparable_dates: int,
    max_pairs: int,
    max_pairs_per_key: int,
) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """Compara pares e separa atualização do mesmo registro de novo atendimento.

    Hierarquia conservadora:
    1. conflito em identidade estável;
    2. cronologia claramente distante/ano administrativo distinto;
    3. atualização de classificação ou punção quando há âncora forte do mesmo registro;
    4. duplicidade provável com identidade suficiente e cronologia curta;
    5. divergências sem âncora seguem para revisão manual.
    """
    close_days = max(0, int(close_days))
    distinct_days = max(close_days + 1, int(distinct_days))
    min_comparable_dates = max(1, min(len(SINAN_OVERLAP_DATE_FIELDS), int(min_comparable_dates)))
    max_pairs = max(1, int(max_pairs))
    max_pairs_per_key = max(1, int(max_pairs_per_key))

    coverage_columns = [
        "chaves_candidatas_repetidas",
        "registros_candidatos",
        "pares_teoricos",
        "pares_classificados",
        "pares_omitidos_por_limite",
        "chaves_com_pares_limitados",
        "limite_total_pares",
        "limite_pares_por_chave",
    ]
    if records is None or records.empty:
        return pd.DataFrame(), pd.DataFrame([{col: 0 for col in coverage_columns}])
    work = records.copy()
    if "chave_candidata" not in work.columns:
        return pd.DataFrame(), pd.DataFrame([{col: 0 for col in coverage_columns}])

    work = work[work["chave_candidata"].notna()].copy()
    groups = [(str(key), group.copy()) for key, group in work.groupby("chave_candidata", sort=True, dropna=True)]
    groups = [(key, group) for key, group in groups if len(group) > 1]
    theoretical_pairs = int(sum((len(group) * (len(group) - 1)) // 2 for _key, group in groups))
    ...
    for key, group in groups:
        ...
        if sort_cols:
            group = group.sort_values(sort_cols, na_position="last", kind="stable")
        rows = group.to_dict("records")
        group_theoretical = (len(rows) * (len(rows) - 1)) // 2
        group_generated = 0

        for left_idx in range(len(rows) - 1):
            if generated_total >= max_pairs or group_generated >= max_pairs_per_key:
                break
            left = rows[left_idx]
            for right_idx in range(left_idx + 1, len(rows)):
                ...
                name_status, name_similarity, name_near = _overlap_effective_name_status(
                    left.get("nm_pacient"), right.get("nm_pacient")
                )
                nu_status = _overlap_compare(left.get("nu_notific"), right.get("nu_notific"), _overlap_text)
                birth_status, birth_field, birth_date_status, birth_year_status = _overlap_effective_birth_status(left, right)
                sex_status = _overlap_compare(left.get("sexo"), right.get("sexo"), _overlap_sex)
                mother_status, mother_similarity, mother_near = _overlap_effective_name_status(
                    left.get("nm_mae_pac"), right.get("nm_mae_pac")
                )
                notification_year_status = _overlap_compare(
                    left.get("ano_notificacao"), right.get("ano_notificacao"), _overlap_year
                )
                uf_status = _overlap_compare(left.get("uf_notificacao"), right.get("uf_notificacao"), _overlap_code)
                municipality_status = _overlap_compare(
                    left.get("municipio_notificacao"), right.get("municipio_notificacao"), _overlap_code
                )
                unit_status = _overlap_compare(
                    left.get("unidade_notificadora"), right.get("unidade_notificadora"), _overlap_code
                )
                residence_status = _overlap_compare(
                    left.get("municipio_residencia"), right.get("municipio_residencia"), _overlap_code
                )
                puncao_status = _overlap_compare(
                    left.get("lab_puncao_codigo"), right.get("lab_puncao_codigo"), _overlap_yes_no
                )

                classi_kind = _overlap_change_kind(
                    left.get("classi_fin_codigo"), right.get("classi_fin_codigo"), _overlap_code
                )
                con_kind = _overlap_change_kind(
                    left.get("con_diages_codigo"),
                    right.get("con_diages_codigo"),
                    lambda value: _overlap_code(value, pad2=True),
                )
                puncao_kind = _overlap_change_kind(
                    left.get("lab_puncao_codigo"), right.get("lab_puncao_codigo"), _overlap_yes_no
                )
                classi_status = _overlap_compare(
                    left.get("classi_fin_codigo"), right.get("classi_fin_codigo"), _overlap_code
                )
                con_status = _overlap_compare(
                    left.get("con_diages_codigo"),
                    right.get("con_diages_codigo"),
                    lambda value: _overlap_code(value, pad2=True),
                )

                gaps = _overlap_date_gaps(left, right)
                comparable_gaps = {field: gap for field, gap in gaps.items() if gap is not None}
                close_fields = [field for field, gap in comparable_gaps.items() if gap <= close_days]
                distant_fields = [field for field, gap in comparable_gaps.items() if gap > distinct_days]
                intermediate_fields = [
                    field for field, gap in comparable_gaps.items() if close_days < gap <= distinct_days
                ]
                max_gap_field = None
                max_gap = None
                if comparable_gaps:
                    max_gap_field, max_gap = max(comparable_gaps.items(), key=lambda item: (item[1], item[0]))

                exact_record_fields = [
                    field for field in ("DT_DIGITA", "LAB_DTPUNC") if gaps.get(field) == 0
                ]
                strong_exact_record_evidence = bool(exact_record_fields)
                dates_compared = len(comparable_gaps)
                all_dates_close = dates_compared >= min_comparable_dates and len(close_fields) == dates_compared
                any_date_distant = bool(distant_fields)

                required_identity: List[Tuple[str, Optional[bool]]] = [(birth_field, birth_status), ("CS_SEXO", sex_status)]
                if candidate_type == "notification":
                    required_identity.insert(0, ("NM_PACIENT", name_status))
                identity_conflicts = [label for label, status in required_identity if status is False]
                optional_identity_conflicts = ["NM_MAE_PAC"] if mother_status is False else []
                identity_compared = sum(status is not None for _label, status in required_identity)
                identity_equal = sum(status is True for _label, status in required_identity)
                identity_complete_and_equal = bool(required_identity) and all(status is True for _label, status in required_identity)
                identity_compatible = not identity_conflicts
                identity_support_score = identity_equal + (1 if mother_status is True else 0)
                identity_sufficient = identity_complete_and_equal or (
                    identity_compatible and strong_exact_record_evidence and identity_support_score >= 2
                )

                context_conflicts: List[str] = []
                for label, status in (
                    ("ANO_NOTIFICACAO", notification_year_status),
                    ("UF_NOTIFICACAO", uf_status),
                    ("MUNICIPIO_NOTIFICACAO", municipality_status),
                    ("UNIDADE_NOTIFICADORA", unit_status),
                ):
                    if status is False:
                        context_conflicts.append(label)
                strong_context_conflict = (
                    notification_year_status is False
                    or uf_status is False
                    or (municipality_status is False and unit_status is False)
                ) and not strong_exact_record_evidence

                classification_change_fields: List[str] = []
                classification_conflict_fields: List[str] = []
                for label, kind in (("CLASSI_FIN", classi_kind), ("CON_DIAGES", con_kind)):
                    if kind in {"diferente", "preenchimento_entre_extracoes"}:
                        classification_change_fields.append(label)
                    if kind == "diferente":
                        classification_conflict_fields.append(label)
                puncture_changed = puncao_kind in {"diferente", "preenchimento_entre_extracoes"}

                same_notification_scope = (
                    nu_status is True
                    and notification_year_status is not False
                    and (uf_status is not False or strong_exact_record_evidence)
                )
                name_candidate_support = (
                    candidate_type != "name"
                    or same_notification_scope
                    or (
                        identity_complete_and_equal
                        and all_dates_close
                        and dates_compared >= max(2, min_comparable_dates)
                        and (birth_date_status is True or mother_status is True)
                    )
                )
                temporal_anchor = all_dates_close or strong_exact_record_evidence
                same_record_anchor = (
                    same_notification_scope
                    and identity_compatible
                    and identity_sufficient
                    and temporal_anchor
                    and name_candidate_support
                )

                if identity_conflicts:
                    if candidate_type == "notification":
                        classification = "Reuso de numeração provável — outro paciente"
                    else:
                        classification = "Repetição de nome provável — homônimo/outro paciente"
                    broad_class = "Não duplicidade provável"
                    main_reason = "Conflito de identidade em " + ", ".join(identity_conflicts) + "."
                elif any_date_distant or notification_year_status is False:
                    if candidate_type == "notification":
                        classification = "Reuso de numeração provável — outro atendimento"
                    else:
                        classification = "Repetição de nome provável — outro atendimento"
                    broad_class = "Não duplicidade provável"
                    if distant_fields:
                        main_reason = (
                            f"Diferença temporal superior a {distinct_days} dias em "
                            + ", ".join(distant_fields)
                            + "."
                        )
                    else:
                        main_reason = "Ano da notificação divergente entre os registros."
                elif classification_change_fields and same_record_anchor:
                    classification = "Provável mesmo registro — classificação atualizada entre extrações"
                    broad_class = "Duplicidade provável"
                    main_reason = (
                        "Mesmo NU_NOTIFIC/escopo, identidade compatível e âncora temporal do mesmo registro; "
                        + ", ".join(classification_change_fields)
                        + " foi preenchido ou refinado entre extrações."
                    )
                elif puncture_changed and same_record_anchor:
                    classification = "Provável mesmo registro — investigação/punção atualizada entre extrações"
                    broad_class = "Duplicidade provável"
                    main_reason = (
                        "Mesmo NU_NOTIFIC/escopo, identidade compatível e cronologia curta; LAB_PUNCAO mudou entre extrações."
                    )
                elif optional_identity_conflicts and not strong_exact_record_evidence:
                    classification = "Possível duplicidade — revisão manual"
                    broad_class = "Revisão manual"
                    main_reason = "NM_MAE_PAC diverge; por ser evidência complementar, o par foi preservado para revisão manual."
                elif temporal_anchor and identity_sufficient and name_candidate_support:
                    classification = "Duplicidade provável"
                    broad_class = "Duplicidade provável"
                    anchor_text = (
                        "sinal exato em " + ", ".join(exact_record_fields)
                        if strong_exact_record_evidence
                        else f"{dates_compared} campo(s) temporal(is) dentro de {close_days} dia(s)"
                    )
                    main_reason = f"Identidade compatível e {anchor_text}, sem evidência forte de episódio distinto."
                elif classification_change_fields:
                    if strong_context_conflict:
                        ...
                        broad_class = "Não duplicidade provável"
                        main_reason = (
                            "Mudança em " + ", ".join(classification_change_fields)
                            + " acompanhada de conflito forte no contexto administrativo."
                        )
                    else:
                        classification = "Classificação divergente — revisão manual"
                        broad_class = "Revisão manual"
                        main_reason = (
                            "Mudança em " + ", ".join(classification_change_fields)
                            + " sem datas/identidade suficientes para decidir entre atualização e novo atendimento."
                        )
                elif puncture_changed:
                    if strong_context_conflict:
                        if candidate_type == "notification":
                            classification = "Reuso de numeração provável — outro atendimento"
                        else:
                            classification = "Repetição de nome provável — outro atendimento"
                        broad_class = "Não duplicidade provável"
                        main_reason = "LAB_PUNCAO diverge junto com conflito forte de contexto administrativo."
                    else:
                        classification = "Investigação/punção divergente — revisão manual"
                        broad_class = "Revisão manual"
                        main_reason = "LAB_PUNCAO diverge, mas a cronologia não permite separar atualização de novo episódio."
                elif dates_compared == 0 and identity_compared == 0:
                    classification = "Indeterminado — dados insuficientes"
                    broad_class = "Revisão manual"
                    main_reason = "Sem datas comparáveis e sem identidade demográfica suficiente."
                elif dates_compared < min_comparable_dates and not strong_exact_record_evidence:
                    classification = "Indeterminado — dados insuficientes"
                    broad_class = "Revisão manual"
                    main_reason = (
                        f"Somente {dates_compared} data(s) comparável(is); o mínimo configurado é {min_comparable_dates}."
                    )
                else:
                    classification = "Possível duplicidade — revisão manual"
                    broad_class = "Revisão manual"
                    if intermediate_fields:
                        main_reason = "Diferença temporal intermediária em " + ", ".join(intermediate_fields) + "."
                    elif context_conflicts:
                        main_reason = "Contexto administrativo divergente em " + ", ".join(context_conflicts) + "."
                    elif not identity_sufficient:
                        main_reason = "Identidade sem conflito, porém incompleta para confirmar o mesmo paciente."
                    elif candidate_type == "name" and not name_candidate_support:
                        main_reason = "Nome e identidade compatíveis, mas NU_NOTIFIC difere e há pouca sustentação temporal."
                    else:
                        main_reason = "Evidências mistas ou incompletas; revisar manualmente."

                priority = "Alta" if broad_class == "Duplicidade provável" else "Média" if broad_class == "Revisão manual" else "Baixa"
                temporal_evidence = "; ".join(
                    f"{field}={gap} dia(s)" for field, gap in comparable_gaps.items()
                ) or "Nenhuma das seis datas pôde ser comparada"
                identity_evidence = "; ".join(
                    [
                        f"NM_PACIENT={_overlap_status_label(name_status, normalized_name=True, near=name_near)}",
                        f"similaridade_nome={name_similarity if name_similarity is not None else '—'}",
                        f"DT_NASC={_overlap_status_label(birth_date_status)}",
                        f"ANO_NASC={_overlap_status_label(birth_year_status)}",
                        f"CS_SEXO={_overlap_status_label(sex_status)}",
                        f"NM_MAE_PAC={_overlap_status_label(mother_status, normalized_name=True, near=mother_near)}",
                        f"NU_NOTIFIC={_overlap_status_label(nu_status)}",
                    ]
                )
                context_evidence = "; ".join(
                    [
                        f"ANO_NOTIFICACAO={_overlap_status_label(notification_year_status)}",
                        f"UF_NOTIFICACAO={_overlap_status_label(uf_status)}",
                        f"MUNICIPIO_NOTIFICACAO={_overlap_status_label(municipality_status)}",
                        f"UNIDADE_NOTIFICADORA={_overlap_status_label(unit_status)}",
                        f"MUNICIPIO_RESIDENCIA={_overlap_status_label(residence_status)}",
                    ]
                )
                investigation_evidence = "; ".join(
                    [
                        f"CLASSI_FIN={_overlap_change_label(classi_kind)}",
                        f"CON_DIAGES={_overlap_change_label(con_kind)}",
                        f"LAB_PUNCAO={_overlap_change_label(puncao_kind)}",
                    ]
                )

                pair_row: Dict[str, object] = {
                    "classificacao_sobreposicao": classification,
                    "classe_ampla": broad_class,
                    "definicao_operacional": _overlap_pair_definition(classification),
                    "prioridade_revisao": priority,
                    "motivo_principal": main_reason,
                    "chave_candidata": left.get("valor_candidato") or right.get("valor_candidato") or key,
                    "chave_candidata_normalizada": key,
                    "registros_na_chave": len(rows),
                    "id_registro_a": left.get("id_registro_fonte"),
                    "id_registro_b": right.get("id_registro_fonte"),
                    "similaridade_nm_pacient": name_similarity,
                    "similaridade_nm_mae_pac": mother_similarity,
                    "status_nm_pacient": _overlap_status_label(name_status, normalized_name=True, near=name_near),
                    "status_nu_notific": _overlap_status_label(nu_status),
                    "status_dt_nasc": _overlap_status_label(birth_date_status),
                    "status_ano_nasc": _overlap_status_label(birth_year_status),
                    "status_cs_sexo": _overlap_status_label(sex_status),
                    "status_nm_mae_pac": _overlap_status_label(mother_status, normalized_name=True, near=mother_near),
                    "status_ano_notificacao": _overlap_status_label(notification_year_status),
                    "status_uf_notificacao": _overlap_status_label(uf_status),
                    "status_municipio_notificacao": _overlap_status_label(municipality_status),
                    "status_unidade_notificadora": _overlap_status_label(unit_status),
                    "status_municipio_residencia": _overlap_status_label(residence_status),
                    "status_lab_puncao": _overlap_change_label(puncao_kind),
                    "status_classi_fin": _overlap_change_label(classi_kind),
                    "status_con_diages": _overlap_change_label(con_kind),
                    "conflito_identidade": bool(identity_conflicts),
                    "campos_identidade_divergentes": "; ".join(identity_conflicts + optional_identity_conflicts) or "Nenhum",
                    "conflito_contexto_administrativo": bool(context_conflicts),
                    "campos_contexto_divergentes": "; ".join(context_conflicts) or "Nenhum",
                    "mudanca_classificacao_etiologia": bool(classification_change_fields),
                    "campos_classificacao_etiologia_alterados": "; ".join(classification_change_fields) or "Nenhum",
                    "campos_classificacao_etiologia_divergentes": "; ".join(classification_conflict_fields) or "Nenhum",
                    "mudanca_lab_puncao": puncture_changed,
                    "sinal_exato_mesmo_registro": bool(exact_record_fields),
                    "campos_exatos_mesmo_registro": "; ".join(exact_record_fields) or "Nenhum",
                    "campos_identidade_comparados": identity_compared,
                    "campos_identidade_concordantes": identity_equal,
                    "datas_comparaveis": dates_compared,
                    "datas_proximas": len(close_fields),
                    "datas_intermediarias": len(intermediate_fields),
                    "datas_distantes": len(distant_fields),
                    "maior_diferenca_dias": max_gap,
                    "campo_maior_diferenca": max_gap_field,
                    "campos_temporais_proximos": "; ".join(close_fields) or "Nenhum",
                    "campos_temporais_intermediarios": "; ".join(intermediate_fields) or "Nenhum",
                    "campos_temporais_distantes": "; ".join(distant_fields) or "Nenhum",
                    "evidencia_temporal": temporal_evidence,
                    "evidencia_identidade": identity_evidence,
                    "evidencia_contexto_administrativo": context_evidence,
                    "evidencia_classificacao_investigacao": investigation_evidence,
                    "janela_curta_dias": close_days,
                    "limite_outro_atendimento_dias": distinct_days,
                    "minimo_datas_comparaveis": min_comparable_dates,
                }
                for field_label, _column_name in SINAN_OVERLAP_DATE_FIELDS:
                    pair_row[f"dif_dias_{field_label.lower()}"] = gaps[field_label]
                ...
                for column in paired_columns:
                    pair_row[f"{column}_a"] = left.get(column)
                    pair_row[f"{column}_b"] = right.get(column)

                pair_rows.append(pair_row)
                generated_total += 1
                group_generated += 1

        if group_generated < group_theoretical:
            limited_keys += 1

    pairs = pd.DataFrame(pair_rows)
    if not pairs.empty:
        priority_order = {"Alta": 1, "Média": 2, "Baixa": 3}
        broad_order = {"Duplicidade provável": 1, "Revisão manual": 2, "Não duplicidade provável": 3}
        pairs["_ord_prioridade"] = pairs["prioridade_revisao"].map(priority_order).fillna(9)
        pairs["_ord_classe"] = pairs["classe_ampla"].map(broad_order).fillna(9)
        pairs = (
            pairs.sort_values(
                ["_ord_prioridade", "_ord_classe", "chave_candidata_normalizada", "id_registro_a", "id_registro_b"],
                kind="stable",
            )
            .drop(columns=["_ord_prioridade", "_ord_classe"])
            .reset_index(drop=True)
        )
        pairs.insert(0, "ordem_exportacao", np.arange(1, len(pairs) + 1))

    coverage = pd.DataFrame(
        [
            {
                "chaves_candidatas_repetidas": len(groups),
                "registros_candidatos": int(len(work)),
                "pares_teoricos": theoretical_pairs,
                "pares_classificados": int(len(pairs)),
                "pares_omitidos_por_limite": max(0, theoretical_pairs - int(len(pairs))),
                "chaves_com_pares_limitados": limited_keys,
                "limite_total_pares": max_pairs,
                "limite_pares_por_chave": max_pairs_per_key,
            }
        ]
    )
...
def summarize_sinan_overlap_pairs(pairs: pd.DataFrame, *, theoretical_pairs: int) -> pd.DataFrame:
    if pairs is None or pairs.empty:
        return pd.DataFrame()
    summary = (
        pairs.groupby("classificacao_sobreposicao", dropna=False)
        .agg(
            pares=("classificacao_sobreposicao", "size"),
            chaves_candidatas_distintas=("chave_candidata_normalizada", "nunique"),
            mediana_maior_diferenca_dias=("maior_diferenca_dias", "median"),
            max_maior_diferenca_dias=("maior_diferenca_dias", "max"),
            pares_com_conflito_identidade=("conflito_identidade", lambda values: int(pd.Series(values).fillna(False).sum())),
            pares_com_mudanca_classificacao=("mudanca_classificacao_etiologia", lambda values: int(pd.Series(values).fillna(False).sum())),
            pares_com_mudanca_puncao=("mudanca_lab_puncao", lambda values: int(pd.Series(values).fillna(False).sum())),
        )
        .reset_index()
    )
    total_classified = max(1, int(len(pairs)))
    summary["pct_pares_classificados"] = (100.0 * summary["pares"] / total_classified).round(2)
    summary["pct_pares_teoricos"] = np.where(
        theoretical_pairs > 0, (100.0 * summary["pares"] / theoretical_pairs).round(2), np.nan
    )
    summary["mediana_maior_diferenca_dias"] = summary["mediana_maior_diferenca_dias"].round(1)
    summary["definicao_operacional"] = summary["classificacao_sobreposicao"].map(_overlap_pair_definition)
    return summary.sort_values(["pares", "classificacao_sobreposicao"], ascending=[False, True]).reset_index(drop=True)


def _sinan_overlap_simple_summary_table(
    ...
    *,
    ...
    interpretation_suffix = (
        " Nomes são comparados após normalização de acentos, caixa, espaços e pontuação."
        if normalized_names
        else " A chave de notificação é escopada pelo ano quando disponível."
    )
    return pd.DataFrame(
        [
            {"Indicador": "Registros avaliados", "Valor": int(row.get("total_registros", 0) or 0), "Interpretação": "Total após os filtros-base."},
            {"Indicador": f"{display_label} preenchido", "Valor": int(row.get("registros_com_valor", 0) or 0), "Interpretação": "Registros elegíveis para iniciar a triagem." + interpretation_suffix},
            {"Indicador": "Chaves candidatas repetidas", "Valor": int(row.get("valores_com_sobreposicao", 0) or 0), "Interpretação": f"Chaves distintas de {display_label} presentes em dois ou mais registros."},
            {"Indicador": "Registros em chaves repetidas", "Valor": int(row.get("registros_em_sobreposicao", 0) or 0), "Interpretação": "Universo encaminhado à comparação multicritério par a par."},
        ]
    )
...
def _prepare_sinan_overlap_classification_table(
    ...
    *,
    ...
    """Seleciona e ordena a tabela sem remover a subclasse específica da decisão."""
    if class_df is None or class_df.empty:
        return pd.DataFrame() if class_df is None else class_df.copy()
    prepared = class_df.drop(
        columns=[column for column in SINAN_OVERLAP_CLASSIFICATION_COLUMNS_TO_HIDE if column in class_df.columns],
        errors="ignore",
    ).copy()
    repeated_label = "Nome repetido" if candidate_type == "name" else "NU_NOTIFIC repetido"
...
def _render_sinan_overlap_pair_block(
    ...
    *,
    ...
    summary = query_sinan_overlap_summary(
        table, target_col, base_where, target_expr_override=target_group_sql
    )
    if summary.empty:
        st.info(f"Sem registros para avaliar `{display_label}` com os filtros atuais.")
        return

    simple_table = _sinan_overlap_simple_summary_table(
        summary, display_label=display_label, normalized_names=candidate_type == "name"
    )
    row = summary.iloc[0]
    c1, c2, c3, c4 = st.columns(4)
    c1.metric("Registros avaliados", f"{int(row['total_registros']):,}".replace(",", "."))
    c2.metric(f"{display_label} preenchido", f"{int(row['registros_com_valor']):,}".replace(",", "."))
    c3.metric("Chaves repetidas", f"{int(row['valores_com_sobreposicao']):,}".replace(",", "."))
    ...
    st.markdown("**Resumo da triagem pela chave candidata**")
    copyable_dataframe(simple_table, width="stretch", hide_index=True)
    download_button(
        simple_table,
        f"sinan_{file_slug}_triagem_chave_candidata_resumo.csv",
        label="Baixar resumo da triagem",
        max_rows=0,
    )

    records = query_sinan_overlap_candidate_records(
        table, target_col, base_where, exprs, context_cols, candidate_type=candidate_type
    )
    if records.empty:
    ...
    st.caption(
        "Relação completa, com uma linha por registro candidato. Inclui DT_NASC, NM_MAE_PAC, sexo válido, "
        "as seis datas (incluindo LAB_DTPUNC), LAB_PUNCAO, CLASSI_FIN/CON_DIAGES e o contexto administrativo disponível."
    )
    ...
    st.caption(
        "Cada linha corresponde a um par. DT_NASC completa prevalece sobre ANO_NASC; sexo ignorado não conta como concordância. "
        "DT_DIGITA exata e LAB_DTPUNC exata são sinais fortes do mesmo registro. Mudanças em CLASSI_FIN/CON_DIAGES ou LAB_PUNCAO "
        "só indicam outro atendimento automaticamente quando acompanhadas de cronologia/contexto incompatível; com identidade e datas "
        "compatíveis, são tratadas como atualização entre extrações."
    )

    pairs, coverage = classify_sinan_overlap_pairs(
        records,
        candidate_type=candidate_type,
        close_days=close_days,
        distinct_days=distinct_days,
        min_comparable_dates=min_comparable_dates,
        max_pairs=max_pairs,
        max_pairs_per_key=max_pairs_per_key,
    )
    coverage_row = coverage.iloc[0] if not coverage.empty else pd.Series(dtype=object)
    theoretical_pairs = int(coverage_row.get("pares_teoricos", 0) or 0)
    classified_pairs = int(coverage_row.get("pares_classificados", 0) or 0)
    omitted_pairs = int(coverage_row.get("pares_omitidos_por_limite", 0) or 0)

    dup_pairs = int((pairs.get("classe_ampla") == "Duplicidade provável").sum()) if not pairs.empty else 0
    nondup_pairs = int((pairs.get("classe_ampla") == "Não duplicidade provável").sum()) if not pairs.empty else 0
    review_pairs = int((pairs.get("classe_ampla") == "Revisão manual").sum()) if not pairs.empty else 0
    updated_pairs = int(
        pairs.get("classificacao_sobreposicao", pd.Series(dtype=str)).astype(str).str.startswith("Provável mesmo registro").sum()
    ) if not pairs.empty else 0
    p1, p2, p3, p4, p5, p6 = st.columns(6)
    p1.metric("Pares teóricos", f"{theoretical_pairs:,}".replace(",", "."))
    p2.metric("Pares classificados", f"{classified_pairs:,}".replace(",", "."))
    p3.metric("Duplicidade provável", f"{dup_pairs:,}".replace(",", "."))
    p4.metric("Mesmo registro atualizado", f"{updated_pairs:,}".replace(",", "."))
    p5.metric("Não duplicidade provável", f"{nondup_pairs:,}".replace(",", "."))
    p6.metric("Revisão manual", f"{review_pairs:,}".replace(",", "."))
    ...
    classification_tables = (
        (
            ...
            "Inclui duplicações sem mudança e prováveis atualizações do mesmo registro entre extrações.",
        ),
        (
            "Não duplicidade provável",
            "nao_duplicidade_provavel",
            "Pares com conflito de identidade ou cronologia/contexto fortemente compatível com episódio distinto.",
        ),
        (
            "Revisão manual",
            "revisao_manual",
            "Pares em que os campos disponíveis não permitem separar com segurança atualização, duplicidade e novo atendimento.",
        ),
    )
    for class_label, class_slug, class_caption in classification_tables:
        ...
        if class_df.empty:
            ...
            continue
        class_df = _prepare_sinan_overlap_classification_table(class_df, candidate_type=candidate_type)
...
def _fuzzy_name_block_signature(value: object) -> str:
    tokens = _overlap_name_tokens(value)
    initials = "".join(sorted(token[0] for token in tokens if token))
    return initials[:6] or ((_overlap_name(value) or "")[:1])


def classify_sinan_fuzzy_name_pairs(
    records: pd.DataFrame,
    *,
    ...
    """Busca opcional de nomes quase iguais com bloqueio por ANO_NASC, sexo e iniciais.

    A saída é sempre revisão manual; o fuzzy amplia cobertura, mas não substitui
    a triagem exata nem promove pares automaticamente a duplicidade.
    """
    if records is None or records.empty:
        return pd.DataFrame(), pd.DataFrame([{"registros_avaliados": 0, "blocos_candidatos": 0, "pares_encontrados": 0}])
    threshold = min(0.999, max(0.80, float(similarity_threshold)))
    max_pairs = max(1, int(max_pairs))
    max_pairs_per_block = max(1, int(max_pairs_per_block))
    work = records.copy()
    work["_nome_norm"] = work.get("nm_pacient", pd.Series(index=work.index, dtype=object)).map(_overlap_name)
    work["_ano_norm"] = work.get("ano_nascimento", pd.Series(index=work.index, dtype=object)).map(_overlap_year)
    work["_sexo_norm"] = work.get("sexo", pd.Series(index=work.index, dtype=object)).map(_overlap_sex)
    work["_bloco_nome"] = work.get("nm_pacient", pd.Series(index=work.index, dtype=object)).map(_fuzzy_name_block_signature)
    work = work[
        work["_nome_norm"].notna()
        & work["_ano_norm"].notna()
        & work["_sexo_norm"].notna()
        & work["_bloco_nome"].astype(str).ne("")
    ].copy()
    groups = [
        (key, group.copy())
        for key, group in work.groupby(["_ano_norm", "_sexo_norm", "_bloco_nome"], sort=True, dropna=True)
        if len(group) > 1
    ]
    ...
    for key, group in groups:
        ...
        theoretical += (len(group_rows) * (len(group_rows) - 1)) // 2
        generated_block = 0
        compared_block = 0
        for i in range(len(group_rows) - 1):
            if compared_total >= max_pairs or compared_block >= max_pairs_per_block:
                break
            for j in range(i + 1, len(group_rows)):
                if compared_total >= max_pairs or compared_block >= max_pairs_per_block:
                    break
                left, right = group_rows[i], group_rows[j]
                compared_total += 1
                compared_block += 1
                if _overlap_name(left.get("nm_pacient")) == _overlap_name(right.get("nm_pacient")):
                    continue
                birth_date_status = _overlap_compare(
                    left.get("data_nascimento"), right.get("data_nascimento"), _overlap_date
                )
                if birth_date_status is False:
                    continue
                similarity = _overlap_name_similarity(left.get("nm_pacient"), right.get("nm_pacient"))
                ...
                gaps = _overlap_date_gaps(left, right)
                comparable = {field: gap for field, gap in gaps.items() if gap is not None}
                close = [field for field, gap in comparable.items() if gap <= 7]
                exact = [field for field in ("DT_DIGITA", "LAB_DTPUNC") if gaps.get(field) == 0]
                priority = "Média" if nu_status is True or exact else "Baixa"
                pair_row: Dict[str, object] = {
                    "classificacao_sobreposicao": "Nome quase igual — revisão manual",
                    "classe_ampla": "Revisão manual",
                    "definicao_operacional": _overlap_pair_definition("Nome quase igual — revisão manual"),
                    "prioridade_revisao": priority,
                    "motivo_principal": (
                        f"Similaridade de nome {similarity:.3f}; ANO_NASC e sexo concordam no bloco. "
                        "Revisar grafia, inversão de sobrenomes e abreviações."
                    ),
                    "bloco_fuzzy": f"ANO={key[0]}|SEXO={key[1]}|INICIAIS={key[2]}",
                    "similaridade_nm_pacient": similarity,
                    "status_nu_notific": _overlap_status_label(nu_status),
                    "status_dt_nasc": _overlap_status_label(birth_date_status),
                    "status_nm_mae_pac": _overlap_status_label(mother_status, normalized_name=True, near=mother_near),
                    "similaridade_nm_mae_pac": mother_similarity,
                    "evidencia_temporal": "; ".join(f"{field}={gap} dia(s)" for field, gap in comparable.items()) or "Sem datas comparáveis",
                    "campos_temporais_proximos": "; ".join(close) or "Nenhum",
                    "campos_exatos_mesmo_registro": "; ".join(exact) or "Nenhum",
                    "id_registro_a": left.get("id_registro_fonte"),
                    "id_registro_b": right.get("id_registro_fonte"),
                }
                for column in (
                    "nu_notific", "nm_pacient", "nm_mae_pac", "data_nascimento", "ano_nascimento", "sexo",
                    "ano_notificacao", "data_notificacao", "data_digitacao", "data_investigacao",
                    "data_internacao", "data_primeiros_sintomas", "data_puncao", "lab_puncao",
                    "classificacao_final", "conclusao_diagnostica", "uf_notificacao",
                    "municipio_notificacao", "unidade_notificadora", "municipio_residencia",
                ):
                    pair_row[f"{column}_a"] = left.get(column)
                    pair_row[f"{column}_b"] = right.get(column)
                rows_out.append(pair_row)
                generated += 1
                generated_block += 1
        if compared_block >= max_pairs_per_block:
            limited_blocks += 1
        if compared_total >= max_pairs:
            break
    pairs = pd.DataFrame(rows_out)
    if not pairs.empty:
        pairs = pairs.sort_values(
            ["prioridade_revisao", "similaridade_nm_pacient"],
    ...
    coverage = pd.DataFrame(
        [
            {
                "registros_avaliados": int(len(work)),
                "blocos_candidatos": len(groups),
                "pares_teoricos_nos_blocos": theoretical,
                "pares_comparados": compared_total,
                "pares_encontrados": int(len(pairs)),
                "blocos_limitados": limited_blocks,
                "limite_total_comparacoes": max_pairs,
                "limite_comparacoes_por_bloco": max_pairs_per_block,
...
def _render_sinan_fuzzy_name_block(
    table: LoadedTable,
    base_where: str,
    exprs: Dict[str, Optional[str]],
    context_cols: Dict[str, Optional[str]],
    *,
    ...
    st.markdown("### Comparação opcional de nomes quase iguais")
    st.caption(
        "Busca de menor prioridade para erros de digitação, abreviações e inversão de nomes. Usa bloqueio por ANO_NASC, "
        "sexo válido e assinatura de iniciais; todos os resultados permanecem em revisão manual."
    )
    records = query_sinan_overlap_candidate_records(
        table,
        name_col,
        ...
        context_cols,
        candidate_type="name",
        only_repeated_keys=False,
        limit=max_records,
    )
    if records.empty:
        st.info("Não há registros com nome suficiente para a triagem fuzzy no recorte atual.")
        return
    if len(records) >= max_records:
        st.warning(
            f"A triagem fuzzy foi limitada aos primeiros {max_records:,} registros do recorte. "
            "Reduza filtros ou aumente o limite com cautela para ampliar a cobertura."
            .replace(",", ".")
        )
    pairs, coverage = classify_sinan_fuzzy_name_pairs(
        records,
        similarity_threshold=similarity_threshold,
        max_pairs=max_pairs,
        max_pairs_per_block=max_pairs_per_block,
    )
    row = coverage.iloc[0] if not coverage.empty else pd.Series(dtype=object)
    c1, c2, c3, c4 = st.columns(4)
    c1.metric("Registros elegíveis", f"{int(row.get('registros_avaliados', 0) or 0):,}".replace(",", "."))
    c2.metric("Blocos candidatos", f"{int(row.get('blocos_candidatos', 0) or 0):,}".replace(",", "."))
    c3.metric("Pares quase iguais", f"{int(row.get('pares_encontrados', 0) or 0):,}".replace(",", "."))
    c4.metric("Limiar", f"{float(row.get('limiar_similaridade', similarity_threshold)):.2f}")
    if pairs.empty:
        st.info("Nenhum nome quase igual atingiu o limiar configurado dentro dos blocos demográficos.")
        return
    copyable_dataframe(pairs, width="stretch", hide_index=True)
    download_button(
        pairs,
        "sinan_nm_pacient_pares_fuzzy_revisao_manual.csv",
        label="Baixar CSV — nomes quase iguais",
        max_rows=0,
    )

# -----------------------------------------------------------------------------
# Compatibilidade com a auditoria administrativa anterior (v49r4).
# As funções abaixo permanecem disponíveis para consultas legadas, mas a área
# "Análise de possível sobreposição de casos" chama exclusivamente a rotina
# multicritério par a par definida acima.
# -----------------------------------------------------------------------------
@dataclass(frozen=True)
...
def _normalized_identity_expr(col: Optional[str]) -> str:
    """Normaliza acentos, caixa e pontuação para comparação exata estável."""
    if not col:
        return "NULL"
    raw = clean_str_expr(col)
    folded = (
        f"translate(UPPER({raw}), "
        "'ÁÀÂÃÄÉÈÊËÍÌÎÏÓÒÔÕÖÚÙÛÜÇÑ', "
        "'AAAAAEEEEIIIIOOOOOUUUUCN')"
    )
    return f"NULLIF(regexp_replace({folded}, '[^0-9A-Z]', '', 'g'), '')"
...
def _sinan_overlap_reference_table(
    ...
    """Documenta campos detectados e o efeito efetivo de cada evidência."""
    detected = {str(col).upper() for col in columns}
    rows = [
        (
            "Chave inicial",
            "NU_NOTIFIC + ano da notificação",
            context_cols.get("nu_notific"),
            "NU_NOTIFIC inicia a triagem; NU_ANO ou o ano de DT_NOTIFIC delimita o escopo quando disponível.",
            "Evita tratar a numeração como globalmente única em todo o estado/período.",
        ),
        (
            "Chave inicial",
            "NU_ANO / DT_NOTIFIC",
            context_cols.get("notification_year") or context_cols.get("date_notification"),
            "Define o ano administrativo da chave de notificação.",
            "Números iguais em anos distintos deixam de consumir a comparação par a par por NU_NOTIFIC.",
        ),
        (
            "Chave inicial",
            "NM_PACIENT",
            context_cols.get("patient"),
            "Forma pares exatos após remover acentos, caixa, espaços e pontuação; a busca fuzzy é opcional.",
            "Nome quase igual nunca vira duplicidade automática: segue para revisão manual.",
        ),
        (
            "Identidade",
            "DT_NASC",
            context_cols.get("birth_date"),
            "Data completa de nascimento, quando disponível.",
            "Prevalece sobre o ano; divergência entre datas completas afasta duplicidade.",
        ),
        (
            "Identidade",
            "ANO_NASC",
            context_cols.get("birth_year") or context_cols.get("birth_date"),
            "Fallback quando DT_NASC não pode ser comparada nos dois registros.",
            "Concordância reforça identidade; divergência afasta duplicidade.",
        ),
        (
            "Identidade complementar",
            "NM_MAE_PAC",
            context_cols.get("mother_name"),
            "Evidência discriminante adicional, sem obrigatoriedade de preenchimento.",
            "Concordância reforça o mesmo paciente; divergência isolada encaminha para revisão, não decide sozinha.",
        ),
        (
            "Identidade",
            "CS_SEXO",
            context_cols.get("sex"),
            "Sexo válido do paciente; I, 0, 9 e textos de ignorado são convertidos em ausência.",
            "Dois valores ignorados não contam como concordância de identidade.",
        ),
        (
            "Cronologia",
            "DT_NOTIFIC",
            context_cols.get("date_notification"),
            "Data de notificação e fonte alternativa do ano da chave.",
            "Diferença absoluta em dias; distância longa favorece episódio distinto.",
        ),
        (
            "Cronologia forte",
            "DT_DIGITA",
            context_cols.get("date_entry"),
            "Data da primeira digitação/inclusão.",
            "Correspondência exata é sinal forte de que se trata da mesma linha/registro entre extrações.",
        ),
        (
            "Cronologia",
            "DT_INVEST",
            context_cols.get("date_investigation"),
            "Início da investigação.",
            "Diferença absoluta em dias entre os registros.",
        ),
        (
            "Cronologia",
            "ATE_INTERN",
            context_cols.get("date_hospitalization"),
            "Data da internação.",
            "Diferença absoluta em dias; distância longa favorece outro atendimento.",
        ),
        (
            "Cronologia",
            "DT_SIN_PRI",
            context_cols.get("date_symptom"),
            "Data dos primeiros sintomas.",
            "Marcador central de novo episódio quando temporalmente distante.",
        ),
        (
            "Punção",
            "LAB_PUNCAO",
            context_cols.get("lab_puncture"),
            "Realização informada da punção lombar/laboratorial.",
            "Mudança com cronologia compatível pode ser atualização; com contexto/datas incompatíveis reforça novo episódio.",
        ),
        (
            "Cronologia forte",
            "LAB_DTPUNC",
            context_cols.get("date_puncture"),
            "Data da punção, aceitando aliases DT_PUNCA/DT_PUNCAO.",
            "Entra na janela temporal; correspondência exata reforça fortemente o mesmo registro.",
        ),
        (
            "Classificação/etiologia",
            "CLASSI_FIN",
            context_cols.get("classification_final"),
            "Classificação final do caso.",
            "Mudança não significa automaticamente outro atendimento; pode formar a classe de registro atualizado.",
        ),
        (
            "Classificação/etiologia",
            "CON_DIAGES",
            context_cols.get("diagnosis_conclusion"),
            "Conclusão diagnóstica/etiológica específica.",
            "Refinamento entre extrações é compatível com atualização quando identidade e cronologia sustentam o mesmo registro.",
        ),
        (
            "Contexto administrativo",
            "UF notificadora",
            context_cols.get("uf_notification"),
            "UF da unidade notificadora, quando presente.",
            "Concordância reforça contexto; divergência compõe conflito administrativo, sem decidir isoladamente.",
        ),
        (
            "Contexto administrativo",
            "ID_MUNICIP",
            context_cols.get("municipality_notification"),
            "Município de notificação.",
            "Concordância reforça o par; divergência é explicitada e ganha peso quando combinada com unidade/ano/datas.",
        ),
        (
            "Contexto administrativo",
            "ID_UNIDADE",
            context_cols.get("notification_unit"),
            "Unidade notificadora/CNES.",
            "Ajuda a distinguir reuso, transferência, renotificação e atualização de exportação.",
        ),
        (
            "Contexto administrativo",
            "ID_MN_RESI",
            context_cols.get("municipality_residence"),
            "Município de residência.",
            "Evidência contextual complementar; nunca decide isoladamente.",
        ),
        (
            "Contexto secundário",
            "DT_ENCERRA / CRITERIO / EVOLUCAO",
            context_cols.get("date_closure") or context_cols.get("confirmation_criterion") or context_cols.get("outcome"),
            "Mantidos na relação completa para auditoria do fluxo da investigação.",
            "Não são cortes automáticos da classificação principal.",
        ),
    ]
    output_rows: List[Dict[str, object]] = []
    for group, field, actual, role, effect in rows:
        actual_text = str(actual) if actual else "—"
        is_detected = bool(actual and str(actual).upper() in detected)
        output_rows.append(
            {
                "Grupo": group,
                "Campo de referência": field,
                "Coluna detectada": actual_text,
                "Disponível": "Sim" if is_detected else "Não",
                "Papel na auditoria": role,
                "Efeito na classificação": effect,
            }
        )
...
def render_sinan_overlap_tab(table: LoadedTable, base_where: str, exprs: Dict[str, Optional[str]]) -> None:
    st.caption(
        "A área usa `NU_NOTIFIC` e `NM_PACIENT` para formar conjuntos candidatos, mas decide par a par com identidade ampliada, "
        "seis datas, classificação/etiologia, punção e contexto administrativo. `NU_NOTIFIC` é escopado pelo ano quando possível; "
        "o resultado é uma triagem auditável e não exclui registros automaticamente."
    )
    schema = schema_df(table)
    columns = schema["coluna"].astype(str).tolist() if "coluna" in schema.columns else []

    nu_col = choose_candidate(columns, ["NU_NOTIFIC", "NU_NOTFIC", "NUM_NOTIFIC", "NUNOTIFIC", "NU_NOTIF"])
    nm_col = choose_candidate(columns, ["NM_PACIENT", "NOME_PACIENTE", "NM_PACIENTE", "PACIENTE"])
    mother_col = choose_candidate(columns, ["NM_MAE_PAC", "NM_MAE", "NOME_MAE_PACIENTE", "NOME_MAE"])
    notification_year_col = choose_candidate(columns, ["NU_ANO", "ANO_NOTIFIC", "ANO_NOTIF", "ANO_NOTIFICACAO"])
    dt_not_col = choose_candidate(columns, ["DT_NOTIFIC", "DT_NOTIF", "DT_NOTIFICACAO", "DATA_NOTIFICACAO"])
    dt_entry_col = choose_candidate(columns, ["DT_DIGITA", "DT_DIGIT", "DT_DIGITACAO", "DATA_DIGITACAO"])
    dt_invest_col = choose_candidate(columns, ["DT_INVEST", "DT_INVESTIGACAO", "DATA_INVESTIGACAO"])
    dt_hospitalization_col = choose_candidate(columns, ["ATE_INTERN", "DT_INTERN", "DT_INTERNACAO", "DATA_INTERNACAO"])
    dt_symptom_col = choose_candidate(columns, ["DT_SIN_PRI", "DT_SINTOMAS", "DATA_PRIMEIROS_SINTOMAS"])
    dt_puncture_col = choose_candidate(
        columns,
        ["LAB_DTPUNC", "LAB_DTPUN", "DT_PUNCA", "DT_PUNCAO", "DATA_PUNCAO", "DATA_DA_PUNCAO", "DT_COLETA_LCR"],
    )
    dt_closure_col = choose_candidate(columns, ["DT_ENCERRA", "DT_ENCERRAMENTO", "DATA_ENCERRAMENTO"])
    birth_year_col = choose_candidate(columns, ["ANO_NASC", "ANO_NASCIMENTO", "NU_ANO_NASC", "ANO_NASC_PAC"])
    ...
    lab_puncture_col = choose_candidate(columns, ["LAB_PUNCAO", "PUNCAO", "PUNCAO_LCR", "PUNCAO_LOMBAR"])
    agravo_col = choose_candidate(columns, ["ID_AGRAVO", "CO_CID", "AGRAVO", "CID"])
    uf_not_col = choose_candidate(columns, ["SG_UF_NOT", "UF_NOTIFIC", "UF_NOT", "CO_UF_NOT", "ID_UF_NOT"])
    ...
    date_coverage = query_sinan_overlap_date_coverage(table, base_where, exprs, context_cols)
    populated_date_fields = (
        int(date_coverage["Disponível no recorte"].eq("Sim").sum())
        if not date_coverage.empty and "Disponível no recorte" in date_coverage.columns
        else 0
    )
    max_reachable_dates = max(1, min(len(SINAN_OVERLAP_DATE_FIELDS), populated_date_fields))
    min_dates_key = "sinan_overlap_min_dates"
    stored_min_dates = int(st.session_state.get(min_dates_key, DEFAULT_SINAN_OVERLAP_MIN_DATES))
    if stored_min_dates < 1 or stored_min_dates > max_reachable_dates:
        st.session_state[min_dates_key] = min(max(1, stored_min_dates), max_reachable_dates)

    with st.expander("Parâmetros da classificação multicritério", expanded=True):
        c1, c2, c3 = st.columns(3)
        close_days = int(
            c1.number_input(
                "Janela curta que favorece duplicidade (dias)",
                min_value=0,
                max_value=365,
                value=int(st.session_state.get("sinan_overlap_close_days", DEFAULT_SINAN_OVERLAP_CLOSE_DAYS)),
                step=1,
                key="sinan_overlap_close_days",
                help="As datas comparáveis devem ficar dentro desta janela para favorecer o mesmo episódio.",
            )
        )
        distinct_days_input = int(
            c2.number_input(
                "Diferença que favorece outro atendimento (dias)",
                min_value=1,
                max_value=3650,
                value=int(st.session_state.get("sinan_overlap_distinct_days", DEFAULT_SINAN_OVERLAP_DISTINCT_DAYS)),
                step=1,
                key="sinan_overlap_distinct_days",
                help="Qualquer uma das seis datas acima deste limite favorece episódio distinto, após avaliar identidade.",
            )
        )
        min_dates = int(
            c3.number_input(
                "Mínimo de datas comparáveis para duplicidade provável",
                min_value=1,
                max_value=max_reachable_dates,
                value=int(st.session_state.get(min_dates_key, DEFAULT_SINAN_OVERLAP_MIN_DATES)),
                step=1,
                key=min_dates_key,
                help="O máximo é ajustado automaticamente à quantidade de campos temporais com algum preenchimento no recorte.",
            )
        )
        distinct_days = max(close_days + 1, distinct_days_input)
        if distinct_days_input <= close_days:
            st.warning(
                f"O limite de outro atendimento precisa ser maior que a janela curta; nesta execução foi ajustado para {distinct_days} dias."
            )
        if populated_date_fields == 0:
            st.warning(
                "Nenhum dos seis campos temporais tem valor no recorte. A classificação dependerá de identidade/contexto e tenderá à revisão manual."
            )
        else:
            st.caption(
                f"Foram detectados {populated_date_fields} de {len(SINAN_OVERLAP_DATE_FIELDS)} campos temporais com algum preenchimento. "
                f"O mínimo padrão foi definido como {DEFAULT_SINAN_OVERLAP_MIN_DATES}."
            )
    ...
    if not nu_col:
        ...
        _render_sinan_overlap_pair_block(
            table,
            base_where,
            exprs,
            context_cols,
            ...
            classification_heading="Duplicidade provável × reuso de numeração",
            intro_caption=(
                "A chave candidata combina `NU_NOTIFIC` com o ano da notificação quando disponível, reduzindo reuso normal entre anos. "
                "Dentro do mesmo escopo, o par é comparado com identidade ampliada, seis datas, punção, classificação e contexto administrativo."
            ),
            close_days=close_days,
            distinct_days=distinct_days,
            min_comparable_dates=min_dates,
            max_pairs=max_pairs,
            max_pairs_per_key=max_pairs_per_key,
        )
    ...
    if not nm_col:
        st.warning("Não localizei o campo `NM_PACIENT` no SINAN carregado.")
    else:
        _render_sinan_overlap_pair_block(
            table,
            base_where,
            ...
            classification_heading="Duplicidade provável × repetição de nome",
            intro_caption=(
                "A triagem principal agrupa nomes iguais após normalização de acentos, caixa, espaços e pontuação. Homônimos e novos episódios "
                "são avaliados por DT_NASC/ANO_NASC, sexo válido, NM_MAE_PAC, NU_NOTIFIC escopado, seis datas, punção e contexto territorial."
            ),
            close_days=close_days,
            distinct_days=distinct_days,
            min_comparable_dates=min_dates,
    ...
    with st.expander("Campos de referência e efeito na classificação", expanded=False):
        st.caption(
            "A tabela documenta a coluna efetivamente detectada, os aliases aceitos e o papel de cada campo. "
            "Valores ausentes ou ignorados nunca são transformados em concordância artificial."
        )
        ...
        download_button(
            reference_df,
            "sinan_sobreposicao_campos_referencia_multicriterio.csv",
            label="Baixar tabela de campos e regras",
            max_rows=0,
        )
    ...
    if source == "SINAN":
        ...
        if not assistencia.empty:
            ...
            fig_assistencia = px.bar(
                ...
                barmode="group",
                text="texto",
                title="Ocorrência de hospitalização por meningite",
...
casos de indivíduos com até 2 anos (≤ 24 meses)."
        ...
        if communicants_col and exprs.get("classi_code") and exprs.get("dt"):
            ...
            if not comunicantes_ident.empty:
                ...
                comunicantes_ident_plot = comunicantes_ident_plot.dropna(subset=["media"])
...
def render_cid_tab(table: LoadedTable, source: str, graph_where: str, exprs: Dict[str, Optional[str]], base_where: Optional[str] = None) -> None:
    ...
    classi_code = exprs.get("classi_code")
    criterio_code = exprs.get("criterio_code")
    if classi_code and criterio_code:
        criterio_presenca = query_sinan_criterion_presence_by_classification(
            table,
            classi_code,
            criterio_code,
            conversion_base_where,
        )
        if criterio_presenca.empty or pd.to_numeric(
            criterio_presenca.get("denominador"), errors="coerce"
        ).fillna(0).max() <= 0:
            st.info("Não há casos com CLASSI_FIN válido (1 ou 2) no recorte atual.")
        else:
            criterio_presenca = criterio_presenca.copy()
            criterio_presenca["texto"] = [
                f"{br_int(n)} ({br_pct(pct)})"
                for n, pct in zip(criterio_presenca["n"], criterio_presenca["pct"])
            ]
            ...
            fig_criterio_presenca = px.bar(
                ...
                color="presenca",
                text="texto",
                title="Preenchimento válido do campo CRITERIO por classificação final",
                labels={
                    "estrato": "Classificação final",
                    "pct": "% dentro do estrato",
                    "presenca": "Registro no campo CRITERIO",
                    "n": "Casos",
                    "denominador": "Total do estrato",
                },
                ...
                hover_data={
                    ...
                    "pct": ":.2f",
                    "ordem_estrato": False,
                    "ordem_presenca": False,
                },
            )
            fig_criterio_presenca.update_layout(barmode="stack", yaxis={"range": [0, 100]})
            fig_criterio_presenca.update_traces(textposition="inside")
            render_plotly_chart(
            ...
            copyable_dataframe(display_criterion_presence, width="stretch", hide_index=True)
            download_button(
                display_criterion_presence,
                "sinan_presenca_criterio_por_classificacao_final.csv",
            )
    else:
        st.warning(
...
def _sinan_sheet_method_mapping(method_label: str) -> Dict[str, str]:
    spec = SINAN_DIAGNOSTIC_METHOD_SPECS.get(method_label, {})
    if spec.get("criterion_code"):
        return dict(SINAN_CRITERION_DICTIONARY_MAPPING)
    return dict(spec.get("mapping") or {})


def _sinan_sheet_method_expr(
    exprs: Dict[str, Optional[str]],
    method_label: str,
    *,
    raw: bool = False,
    ...
    spec = SINAN_DIAGNOSTIC_METHOD_SPECS.get(method_label, {})
    expr_key = str(spec.get("expr_key") or "")
    if raw:
        raw_key = expr_key[:-5] + "_raw" if expr_key.endswith("_code") else expr_key.replace("_code", "_raw")
        return exprs.get(raw_key) or exprs.get(expr_key)
    return exprs.get(expr_key)
...
def _sinan_sheet_method_presence_condition(
    ...
    if not expr:
        return None
    criterion_code = spec.get("criterion_code")
    if criterion_code:
...
def _sinan_sheet_method_inactive_condition(
    ...
    """Estado explicitamente não realizado/ignorado ou célula vazia."""
    expr = _sinan_sheet_method_expr(exprs, method_label)
    mapping = _sinan_sheet_method_mapping(method_label)
    if not expr or not mapping:
        return None
    inactive_parts = [f"({expr}) IS NULL"]
...
def _sinan_sheet_identifier_select(exprs: Dict[str, Optional[str]]) -> str:
    return ",\n                   ".join([
        f"{exprs.get('nu_notific_raw') or 'CAST(NULL AS VARCHAR)'} AS NU_NOTIFIC",
        f"{exprs.get('nm_pacient_raw') or 'CAST(NULL AS VARCHAR)'} AS NM_PACIENT",
        f"{exprs.get('classi_raw') or exprs.get('classi_code') or 'CAST(NULL AS VARCHAR)'} AS CLASSI_FIN",
        f"{exprs.get('con_raw') or exprs.get('con_code') or 'CAST(NULL AS VARCHAR)'} AS CON_DIAGES",
    ])


def query_sinan_sheet_column_completeness(
    table: LoadedTable,
    exprs: Dict[str, Optional[str]],
    where_sql: str,
    method_label: str,
) -> pd.DataFrame:
    """Distribuição de códigos válidos, vazios e valores fora do dicionário."""
    raw_expr = _sinan_sheet_method_expr(exprs, method_label, raw=True)
    code_expr = _sinan_sheet_method_expr(exprs, method_label)
    mapping = _sinan_sheet_method_mapping(method_label)
    if not raw_expr or not code_expr or not mapping:
        return pd.DataFrame()
    valid_codes_sql = ", ".join(qstr(code) for code in mapping)
    sql = f"""
        WITH base AS (
            SELECT {raw_expr} AS valor_bruto,
                   {code_expr} AS codigo_padronizado
            FROM {table.ref_sql}
            {where_sql}
        ), classificados AS (
            SELECT valor_bruto, codigo_padronizado,
                   CASE
                        WHEN valor_bruto IS NULL THEN 'Célula vazia'
                        WHEN codigo_padronizado IN ({valid_codes_sql}) THEN 'Preenchida corretamente'
                        ELSE 'Preenchida com valor fora do dicionário'
                    END AS categoria,
                   CASE
                        WHEN valor_bruto IS NULL THEN 'Vazia'
                        WHEN codigo_padronizado IN ({valid_codes_sql}) THEN 'Válida'
                        ELSE 'Fora do dicionário'
        ...
        FROM classificados
        CROSS JOIN totais
        GROUP BY categoria, status_preenchimento
    ...
    if result.empty:
        return result
    category_status = [
        ("Preenchida corretamente", "Válida"),
        ("Célula vazia", "Vazia"),
        ("Preenchida com valor fora do dicionário", "Fora do dicionário"),
    ]
    ...
    rows: List[Dict[str, object]] = []
    for categoria, status in category_status:
        ...
        rows.append({
            ...
            "pct_total_elegivel": round(100.0 * n / total_elegivel, 2) if total_elegivel else np.nan,
        })
    return pd.DataFrame(rows)


def query_sinan_sheet_method_value_distribution(
    table: LoadedTable,
    exprs: Dict[str, Optional[str]],
    where_sql: str,
    method_label: str,
) -> pd.DataFrame:
    """Detalha todo o domínio oficial do método, incluindo códigos com zero casos."""
    raw_expr = _sinan_sheet_method_expr(exprs, method_label, raw=True)
    code_expr = _sinan_sheet_method_expr(exprs, method_label)
    mapping = _sinan_sheet_method_mapping(method_label)
    if not raw_expr or not code_expr or not mapping:
        return pd.DataFrame()
    valid_codes_sql = ", ".join(qstr(code) for code in mapping)
    label_case = "CASE " + " ".join(
        f"WHEN codigo_padronizado = {qstr(code)} THEN {qstr(code + ' — ' + label)}"
    ...
    sql = f"""
        WITH base AS (
            ...
            SELECT CASE
                       WHEN valor_bruto IS NULL THEN 'Célula vazia'
                       WHEN codigo_padronizado IN ({valid_codes_sql}) THEN {label_case}
                       ELSE 'Valor fora do dicionário'
                   END AS categoria,
                   CASE
                       WHEN valor_bruto IS NULL THEN 'Vazia'
                       WHEN codigo_padronizado IN ({valid_codes_sql}) THEN 'Válida'
            ...
            SELECT COUNT(*) AS total_elegivel,
                   COUNT(*) FILTER (WHERE status_preenchimento = 'Válida') AS total_validamente_preenchido
            FROM classificados
        )
        SELECT categoria,
               status_preenchimento,
               COUNT(*) AS n,
               MAX(total_elegivel) AS total_elegivel,
               MAX(total_validamente_preenchido) AS total_validamente_preenchido
        FROM classificados
        CROSS JOIN totais
        GROUP BY categoria, status_preenchimento
    """
    ...
    observed = {
        str(row.categoria): int(getattr(row, "n", 0) or 0)
        for row in result.itertuples(index=False)
    }
    rows: List[Dict[str, object]] = []

    def append_row(codigo: str, categoria: str, status: str) -> None:
        n = observed.get(categoria, 0)
        rows.append({
            "codigo": codigo,
            "categoria": categoria,
            "status_preenchimento": status,
            ...
            "pct_entre_validamente_preenchidos": (
                round(100.0 * n / total_validamente_preenchido, 2)
                if status == "Válida" and total_validamente_preenchido
                else np.nan
            ),
            "pct_total_elegivel": (
                round(100.0 * n / total_elegivel, 2) if total_elegivel else np.nan
            ),
        })

    for code, label in mapping.items():
        append_row(code, f"{code} — {label}", "Válida")
    append_row("", "Célula vazia", "Vazia")
    append_row("", "Valor fora do dicionário", "Fora do dicionário")
    return pd.DataFrame(rows)
...
def query_sinan_sheet_column_errors(
    ...
    """Casos cuja célula está vazia ou fora dos códigos do dicionário."""
    raw_expr = _sinan_sheet_method_expr(exprs, method_label, raw=True)
    code_expr = _sinan_sheet_method_expr(exprs, method_label)
    mapping = _sinan_sheet_method_mapping(method_label)
    if not raw_expr or not code_expr or not mapping:
    ...
    sql = f"""
        ...
        SELECT *,
               ...
               END AS tipo_erro,
               {qstr(', '.join(mapping.keys()))} AS codigos_validos
        FROM avaliados
        WHERE valor_informado IS NULL OR codigo_padronizado NOT IN ({valid_codes_sql})
...
def count_sinan_cases_with_any_performed_diagnostic_method(
    ...
    """Conta registros com ao menos um método laboratorial válido realizado."""
    performed_conditions = [
        condition
        for method_label in SINAN_SINGLE_METHOD_OPTIONS
        if (condition := _sinan_sheet_method_presence_condition(exprs, method_label))
    ]
    if not performed_conditions:
        return 0
    sql = f"""
        SELECT COUNT(*) AS n
        FROM {table.ref_sql}
        {append_clause(where_sql, '(' + ' OR '.join(performed_conditions) + ')')}
    """
    result = run_query(table, sql)
    if result.empty:
        return 0
    return int(pd.to_numeric(result.iloc[0].get("n"), errors="coerce") or 0)


def _sinan_precise_agent_label_case(expr_sql: str) -> str:
    return "CASE " + " ".join(
        f"WHEN {expr_sql} = {qstr(code)} THEN {qstr(label)}"
        for code, label in SINAN_PRECISE_BACTERIAL_AGENT_LABELS.items()
    ) + " ELSE NULL END"


def query_sinan_bacterioscopy_compatibility(
    table: LoadedTable,
    exprs: Dict[str, Optional[str]],
    where_sql: str,
) -> pd.DataFrame:
    """Compatibilidade estrita entre bacterioscopia, culturas, PCR e diagnóstico final."""
    required_keys = [
        "lab_bcliqu_code",
        "lab_ctliqu_code",
        "lab_ctsang_code",
    ...
    if any(not exprs.get(key) for key in required_keys):
        return pd.DataFrame()
    bacter = exprs["lab_bcliqu_code"]
    cult_lcr = exprs["lab_ctliqu_code"]
    cult_sang = exprs["lab_ctsang_code"]
    pcr = exprs["lab_pcliqu_code"]
    con_code = exprs["con_code"]
    precise_codes_sql = ", ".join(qstr(code) for code in SINAN_PRECISE_BACTERIAL_AGENT_LABELS)
    bacter_codes_sql = ", ".join(qstr(code) for code in SINAN_BACTERIOSCOPY_PRECISE_AGENT)
    bacter_agent_case = "CASE " + " ".join(
        f"WHEN {bacter} = {qstr(code)} THEN {qstr(agent)}"
        for code, agent in SINAN_BACTERIOSCOPY_PRECISE_AGENT.items()
    ) + " ELSE NULL END"
    final_agent_case = f"""
        CASE
            WHEN {con_code} IN ('02', '03') THEN '01'
            WHEN {con_code} = '09' THEN '06'
            WHEN {con_code} = '10' THEN '07'
            ELSE NULL
        END
    """
    identifiers = _sinan_sheet_identifier_select(exprs)
    sql = f"""
        WITH fonte AS (
            ...
            SELECT __linha_fonte AS linha_fonte,
                   ...
                   {qstr(table.label or table.source)} AS origem,
                   {identifiers},
                   {bacter} AS bacterioscopia_codigo,
                   {bacter_agent_case} AS bacterioscopia_agente_esperado,
                   {cult_lcr} AS cultura_liquor_codigo,
                   {cult_sang} AS cultura_sangue_codigo,
                   {pcr} AS pcr_liquor_codigo,
                   {final_agent_case} AS diagnostico_final_agente
            FROM fonte
            {where_sql}
        )
        SELECT *,
               {_sinan_precise_agent_label_case('bacterioscopia_agente_esperado')} AS bacterioscopia_agente,
               {_sinan_precise_agent_label_case('cultura_liquor_codigo')} AS cultura_liquor_agente,
               {_sinan_precise_agent_label_case('cultura_sangue_codigo')} AS cultura_sangue_agente,
               {_sinan_precise_agent_label_case('pcr_liquor_codigo')} AS pcr_liquor_agente,
               {_sinan_precise_agent_label_case('diagnostico_final_agente')} AS diagnostico_final,
               CASE WHEN bacterioscopia_agente_esperado = cultura_liquor_codigo THEN 'Compatível' ELSE 'Incompatível' END AS compat_cultura_liquor,
               CASE WHEN bacterioscopia_agente_esperado = cultura_sangue_codigo THEN 'Compatível' ELSE 'Incompatível' END AS compat_cultura_sangue,
               CASE WHEN bacterioscopia_agente_esperado = pcr_liquor_codigo THEN 'Compatível' ELSE 'Incompatível' END AS compat_pcr_liquor,
               CASE WHEN bacterioscopia_agente_esperado = diagnostico_final_agente THEN 'Compatível' ELSE 'Incompatível' END AS compat_diagnostico_final
        FROM base
        WHERE bacterioscopia_codigo IN ({bacter_codes_sql})
          AND cultura_liquor_codigo IN ({precise_codes_sql})
          AND cultura_sangue_codigo IN ({precise_codes_sql})
          AND pcr_liquor_codigo IN ({precise_codes_sql})
          AND diagnostico_final_agente IN ({precise_codes_sql})
        ORDER BY linha_fonte
    """
    return run_query(table, sql)


def _sinan_lcr_completeness_param_expr(
...
def _sinan_lcr_completeness_category_sql(
    ...
    return (
        ...
        raw_expr,
        value_expr,
        "Completude literal: toda célula não vazia conta como preenchida, sem julgamento por faixa clínica ou literatura.",
    )
...
def query_sinan_lcr_parameter_completeness(
    ...
    """Separa células preenchidas e não preenchidas entre casos com punção."""
    ...
    sql = f"""
        ...
        CROSS JOIN totais
        GROUP BY categoria
        ORDER BY CASE categoria WHEN 'Não preenchidos' THEN 1 ELSE 2 END
    """
    result = run_query(table, sql)
    if result.empty:
        return result
    ...
    rows = []
    for categoria in ["Não preenchidos", "Preenchidos"]:
        n = observed.get(categoria, 0)
        rows.append({
            "categoria": categoria,
            "n": n,
            "total_elegivel": total_elegivel,
            "pct_total_elegivel": round(100.0 * n / total_elegivel, 2) if total_elegivel else np.nan,
            "regra_preenchimento": reference_note,
        })
    return pd.DataFrame(rows)
...
def query_sinan_lcr_parameter_cases(
    ...
    """Lista todos os casos elegíveis e sua situação de preenchimento literal."""
    param_key = SINAN_LCR_COMPLETENESS_PARAMS.get(parameter_label)
    specification = _sinan_lcr_completeness_category_sql(exprs, parameter_label)
    ...
    sql = f"""
        WITH fonte AS (
            ...
            SELECT __linha_fonte AS linha_fonte,
                   ...
                   {qstr(reference_note)} AS regra_preenchimento
            FROM fonte
            {where_sql}
        )
        SELECT *
        FROM avaliados
        ORDER BY CASE situacao_preenchimento WHEN 'Não preenchidos' THEN 1 ELSE 2 END,
            linha_fonte
    """
    return run_query(table, sql)
...
def query_sinan_lcr_raincloud_frequency(
    ...
    """Frequência exata de um parâmetro do LCR por grupo analítico."""
    param_key = SINAN_LCR_COMPLETENESS_PARAMS.get(parameter_label)
    value_expr = exprs.get(f"lab_{param_key}") if param_key else None
    classi_expr = exprs.get("classi_raw") or exprs.get("classi_code")
    if not param_key or not value_expr or (not classi_expr and not group_sql):
        return pd.DataFrame()
...
def query_sinan_lcr_aberrant_cases(
    ...
    solicitado (0-100% para diferenciais celulares).
    """
    param_key = SINAN_LCR_COMPLETENESS_PARAMS.get(parameter_label)
    raw_expr = _sinan_lcr_completeness_param_expr(exprs, parameter_label)
    value_expr = exprs.get(f"lab_{param_key}") if param_key else None
    classi_expr = exprs.get("classi_raw") or exprs.get("classi_code")
    if not param_key or not raw_expr or not value_expr or (not classi_expr and not group_sql):
        return pd.DataFrame()
    identifiers = _sinan_sheet_identifier_select(exprs)
    field_name = str(SINAN_QUIMIO_PARAMS.get(param_key, {}).get("default_col", param_key))
    if group_sql is None:
...
def query_sinan_lcr_parameter_coverage(
    ...
    """Completude bruta e disponibilidade numérica por grupo analítico."""
    param_key = SINAN_LCR_COMPLETENESS_PARAMS.get(parameter_label)
    raw_expr = _sinan_lcr_completeness_param_expr(exprs, parameter_label)
    value_expr = exprs.get(f"lab_{param_key}") if param_key else None
    classi_expr = exprs.get("classi_raw") or exprs.get("classi_code")
...
def render_sinan_lcr_raincloud_chart(
    table: LoadedTable,
    exprs: Dict[str, Optional[str]],
    where_sql: str,
    parameter_label: str,
    ...
    *,
    ...
    """Renderiza um único raincloud e suas tabelas para evitar consultas desnecessárias."""
    param_key = SINAN_LCR_COMPLETENESS_PARAMS[parameter_label]
    meta = sinan_lcr_param_metadata(param_key)
    field_name = str(SINAN_QUIMIO_PARAMS[param_key]["default_col"])
    if not exprs.get(f"lab_{param_key}"):
        st.warning(f"O campo `{field_name}` não foi localizado na base carregada.")
        return

    coverage = query_sinan_lcr_parameter_coverage(
    ...
    order_index = {str(group): index for index, group in enumerate(group_order)}
    coverage["__ordem"] = coverage["grupo"].astype(str).map(order_index).fillna(len(order_index))
    coverage = coverage.sort_values(["__ordem", "grupo"]).drop(columns="__ordem")
    coverage["pct_nao_preenchidos"] = pd.to_numeric(coverage["pct_nao_preenchidos"], errors="coerce")
    observed_groups = set(coverage["grupo"].astype(str))
    groups_without_cases = [str(group) for group in group_order if str(group) not in observed_groups]
    groups_without_numeric = coverage.loc[
        pd.to_numeric(coverage["disponiveis_no_raincloud"], errors="coerce").fillna(0).eq(0),
        "grupo",
    ].astype(str).tolist()
    if groups_without_cases:
        st.caption(
            "Categorias previstas sem casos elegíveis após as regras deste recorte: "
            + "; ".join(groups_without_cases)
            + "."
        )
    if groups_without_numeric:
        st.caption(
            "Categorias com casos elegíveis, mas sem valor numérico disponível para este parâmetro — por isso não "
            "é possível desenhar sua densidade: "
            + "; ".join(groups_without_numeric)
            + "."
        )
    coverage_messages = [
    ...
    if fixed_upper_bound is not None:
        if not aberrant_df.empty:
            ...
            download_button(
                aberrant_df,
                f"{file_prefix}_{param_key}_casos_aberrantes.csv",
    ...
    fig, summary, sampled = make_weighted_raincloud_halfeye(
        ...
        group_order,
        value_col="valor",
        xaxis_title=f"{parameter_label} ({unit})",
        value_label=parameter_label,
    ...
    copyable_dataframe(summary, width="stretch", hide_index=True)
    download_button(summary, f"{file_prefix}_{param_key}_resumo.csv", max_rows=len(summary))
    st.markdown(f"**Frequência absoluta de cada valor de {parameter_label.lower()}**")
    copyable_dataframe(frequency, width="stretch", hide_index=True)
    download_button(
        frequency,
        f"{file_prefix}_{param_key}_frequencias_exatas.csv",
        label="Baixar frequências exatas (CSV)",
        max_rows=len(frequency),
    )


def _sinan_lcr_detailed_etiology_group_spec(
    exprs: Dict[str, Optional[str]], analysis_kind: str
) -> Tuple[Optional[str], List[str], Optional[str], List[str]]:
    """Define grupos etiológicos estritos após padronizar códigos do dicionário."""
    classi_code = exprs.get("classi_code")
    con_code = exprs.get("con_code")
    criterio_code = exprs.get("criterio_code")
    required = {
        "CLASSI_FIN": classi_code,
        "CON_DIAGES": con_code,
        "CRITERIO": criterio_code,
    }
    if analysis_kind == "Bacterianas":
        required["CLA_ME_BAC"] = exprs.get("cla_me_bac_code")
    else:
        required["CLA_ME_ASS"] = exprs.get("cla_me_ass_code")
    missing = [name for name, expression in required.items() if not expression]
    if missing:
    ...
    if analysis_kind == "Bacterianas":
        bac_code = exprs["cla_me_bac_code"]
        direct_codes = ["02", "03", "09", "10"]
        direct_codes_sql = ", ".join(qstr(code) for code in direct_codes)
        valid_bac_codes_sql = ", ".join(qstr(code) for code in SINAN_CLA_ME_BAC)
        defined_bac_codes_sql = ", ".join(
            qstr(code) for code in SINAN_CLA_ME_BAC if code not in {"28", "81"}
        )
        group_order = [
            "Meningite bacteriana com etiologia definida",
            "Meningite bacteriana sem etiologia definida",
            "Meningite bacteriana com etiologia não esclarecida",
        ]
        group_sql = f"""
            CASE
                WHEN ({con_code}) IN ({direct_codes_sql})
                    OR (({con_code}) = '05' AND ({bac_code}) IN ({defined_bac_codes_sql}))
                    THEN {qstr(group_order[0])}
                WHEN ({con_code}) = '05' AND ({bac_code}) = '81' THEN {qstr(group_order[1])}
                WHEN ({con_code}) = '05' AND ({bac_code}) = '28' THEN {qstr(group_order[2])}
                ELSE NULL
            END
        """
        eligibility = (
            f"{common_eligibility} AND ({con_code}) IN ('02', '03', '05', '09', '10') AND "
            f"(({con_code}) IN ({direct_codes_sql}) OR "
            f"(({con_code}) = '05' AND ({bac_code}) IN ({valid_bac_codes_sql})))"
        )
        return group_sql, group_order, eligibility, []

    ass_code = exprs["cla_me_ass_code"]
    valid_ass_codes_sql = ", ".join(qstr(code) for code in SINAN_CLA_ME_ASS)
    defined_ass_codes_sql = ", ".join(
        qstr(code) for code in SINAN_CLA_ME_ASS if code not in {"59", "73", "74", "75"}
    )
    group_order = [
        "Meningite asséptica com etiologia definida",
        "Meningite asséptica sem etiologia definida",
        "Meningite asséptica com etiologia não esclarecida",
    ]
    group_sql = f"""
        CASE
            WHEN ({ass_code}) IN ({defined_ass_codes_sql}) THEN {qstr(group_order[0])}
            WHEN ({ass_code}) = '75' THEN {qstr(group_order[1])}
            WHEN ({ass_code}) IN ('59', '73', '74') THEN {qstr(group_order[2])}
            ELSE NULL
        END
    """
    eligibility = (
...
def render_demography_tab(table: LoadedTable, source: str, graph_where: str, exprs: Dict[str, Optional[str]], base_where: Optional[str] = None) -> None:
    ...
    def render_age_raincloud_chart() -> None:
        if not age:
            return
        if source == "SINAN" and classi_code:
            group_sql = f"""
                ...
                END
            """
            rain_where = demography_case_base_where
            group_order = ["Casos confirmados", "Casos descartados"]
            title = "Raincloud half-eye da idade — casos confirmados e descartados"
        else:
            group_label = "Óbitos/registros analisados" if source == "SIM" else "Atendimentos/registros analisados"
            group_sql = qstr(group_label)
            rain_where = graph_where
            group_order = [group_label]
            title = f"Raincloud half-eye da idade — {source}"
        age_frequency = query_age_frequency(table, age, rain_where, group_sql=group_sql)
        ...
        if source != "SINAN":
            st.caption(
                f"O {source} não possui CLASSI_FIN equivalente ao SINAN; por isso não é metodologicamente possível "
                "separar confirmados e descartados nesta base. O gráfico representa o conjunto definido pelos filtros atuais."
            )
        if sampled_rain:
            st.caption(
                "Para manter a interação responsiva, a camada visual de pontos mostra uma amostra sistemática determinística "
        ...
        download_button(
            ...
            label="Baixar frequências exatas usadas no raincloud (CSV)",
            max_rows=len(age_frequency),
        )
...
def render_detailed_lcr_analysis_tab(
    table: LoadedTable,
    exprs: Dict[str, Optional[str]],
    base_where: str,
) -> None:
    """Completude literal e rainclouds dos parâmetros do LCR."""
    puncao_code = exprs.get("puncao_code") or exprs.get("puncao_raw")
    classi_code = exprs.get("classi_code") or exprs.get("classi_raw")
    st.info(
        "Esta seção mantém o recorte de registros com punção lombar realizada (`LAB_PUNCAO = 1`). A completude separa "
        "somente células preenchidas e não preenchidas, sem classificar resultados por faixas da literatura. "
        "Os rainclouds usam apenas valores numericamente interpretáveis e comparam "
    ...
    if not puncao_code:
        st.warning(
            "A seção exige o campo LAB_PUNCAO. Sem ele não é possível restringir a análise aos pacientes "
            "que realizaram punção lombar."
        )
        return
    ...
    with c1:
        case_group = st.selectbox(
            "Estrato de casos",
            group_options,
            key="sinan_lcr_completeness_group",
        )
    with c2:
        parameter_label = st.selectbox(
            "Parâmetro liquórico",
            list(SINAN_LCR_COMPLETENESS_PARAMS.keys()),
            key="sinan_lcr_completeness_parameter",
        )
    ...
    st.caption(
        ...
        "ficou vazio, o registro permanece no denominador. Toda célula não vazia conta como preenchida; esta auditoria "
        "não tenta definir normalidade, plausibilidade clínica ou compatibilidade etiológica."
    )
    ...
    st.caption(
        "As duas categorias são mutuamente exclusivas e usam o mesmo denominador: todos os casos elegíveis com punção. "
        "‘Preenchidos’ significa apenas que a célula não está vazia; não equivale a resultado válido, normalidade ou diagnóstico."
    )

    distribution_export = completeness_df.drop(columns=["texto"], errors="ignore")
    copyable_dataframe(distribution_export, width="stretch", hide_index=True)
    download_button(
        distribution_export,
        f"sinan_liquor_completude_{safe_filename(parameter_label)}.csv",
        max_rows=len(distribution_export),
    )

    st.markdown("**Todos os casos classificados por situação de preenchimento**")
    ...
    if parameter_cases_df.empty:
        ...
        st.caption(
            "A tabela inclui células preenchidas e não preenchidas. `linha_fonte` é a "
    ...
    rain_label = st.selectbox(
        "Parâmetro liquórico do raincloud",
        list(SINAN_LCR_COMPLETENESS_PARAMS.keys()),
    ...
    with c_rain_type:
        detailed_kind = st.selectbox(
            "Grupo etiológico do raincloud",
            ["Bacterianas", "Assépticas"],
            key="sinan_lcr_detailed_etiology_kind",
        )
    with c_rain_parameter:
        detailed_parameter = st.selectbox(
            "Parâmetro liquórico da análise etiológica",
            list(SINAN_LCR_COMPLETENESS_PARAMS.keys()),
            key="sinan_lcr_detailed_etiology_parameter",
        )

    detailed_group_sql, detailed_group_order, eligibility, missing = (
        _sinan_lcr_detailed_etiology_group_spec(exprs, detailed_kind)
    )
    if missing:
        st.warning(
            "Não foi possível montar esta estratificação. Campos ausentes: " + ", ".join(missing) + "."
        )
        return
    ...
    title_detailed = (
        f"Raincloud half-eye de {detailed_parameter.lower()} - meningites {kind_slug} por definição etiológica"
    )
    render_sinan_lcr_raincloud_chart(
        table,
        exprs,
...
def query_sinan_etiology_cases(
    ...
    *,
    ...
    include_aseptic_specification: bool = False,
    include_other_etiology_specification: bool = False,
) -> pd.DataFrame:
    """Lista caso a caso diagnósticos etiológicos definidos por CON_DIAGES.
    ...
    outras bactérias (CON_DIAGES=05), CLA_ME_BAC discrimina o Quadro II; para
    meningite asséptica (CON_DIAGES=07), CLA_ME_ASS discrimina o Quadro III; e
    para outras etiologias (CON_DIAGES=08), CLA_ME_ETI discrimina o Quadro IV.
    CRITERIO é sempre mantido ao lado do diagnóstico. O arquivo de origem nunca é alterado.
    """
    con_code = exprs.get("con_code")
    ...
    ass_code = exprs.get("cla_me_ass_code")
    ass_label = exprs.get("cla_me_ass_label")
    ass_code_select = ass_code or "CAST(NULL AS VARCHAR)"
    ass_label_select = ass_label or "CAST(NULL AS VARCHAR)"
    eti_code = exprs.get("cla_me_eti_code")
    eti_label = exprs.get("cla_me_eti_label")
    eti_code_select = eti_code or "CAST(NULL AS VARCHAR)"
    eti_label_select = eti_label or "CAST(NULL AS VARCHAR)"
    criterio_code_select = exprs.get("criterio_code") or exprs.get("criterio_raw") or "CAST(NULL AS VARCHAR)"
    criterio_label_select = exprs.get("criterio_label") or "CAST(NULL AS VARCHAR)"
    ...
    sql = f"""
        WITH fonte AS (
            ...
            SELECT __linha_fonte AS linha_fonte,
                   ...
                   {ass_code_select} AS CLA_ME_ASS_CODIGO,
                   {ass_label_select} AS agente_asseptico_especificado,
                   {eti_code_select} AS CLA_ME_ETI_CODIGO,
                   {eti_label_select} AS outra_etiologia_especificada,
                   {criterio_code_select} AS CRITERIO_CODIGO,
                   {criterio_label_select} AS criterio_confirmacao
            FROM fonte
            {where_diag}
        )
        SELECT *
        ...
        ORDER BY
            COALESCE(
                ...
                agente_asseptico_especificado,
                outra_etiologia_especificada,
                conclusao_diagnostica
            ),
            NU_NOTIFIC,
            NM_PACIENT,
    ...
    if not include_aseptic_specification:
        result = result.drop(columns=["CLA_ME_ASS_CODIGO", "agente_asseptico_especificado"], errors="ignore")
    if not include_other_etiology_specification:
        result = result.drop(columns=["CLA_ME_ETI_CODIGO", "outra_etiologia_especificada"], errors="ignore")
    return result


def render_detailed_etiology_analysis_tab(
    table: LoadedTable,
    exprs: Dict[str, Optional[str]],
    base_where: str,
) -> None:
    ...
    st.info(
        ...
        "As tabelas caso a caso preservam `NU_NOTIFIC`, nome, linha de origem, código e descrição de `CRITERIO` para "
    ...
    st.markdown("#### Recorte etiológico da série temporal entre confirmados")
    temporal_etiology_group = st.radio(
        "Estratificação etiológica dos critérios ao longo do tempo",
        ["Meningites bacterianas", "Meningites assépticas"],
        horizontal=True,
        key="sinan_detailed_etiology_confirmed_timeseries_group",
    )
    classi_code = exprs.get("classi_code")
    con_code = exprs.get("con_code")
    criterio_code = exprs.get("criterio_code")
    if classi_code and con_code and criterio_code:
        eligible_criterion_codes = [
            code for code in SINAN_CRITERIO if code not in {"04", "06", "07"}
        ]
        eligible_criteria_sql = ", ".join(qstr(code) for code in eligible_criterion_codes)
        con_codes = ["02", "03", "05", "09", "10"] if temporal_etiology_group == "Meningites bacterianas" else ["07"]
        con_codes_sql = ", ".join(qstr(code) for code in con_codes)
        temporal_etiology_where = append_clause(base_where, f"({classi_code}) = '1'")
        temporal_etiology_where = append_clause(
            temporal_etiology_where, f"({con_code}) IN ({con_codes_sql})"
        )
        temporal_etiology_where = append_clause(
            temporal_etiology_where, f"({criterio_code}) IN ({eligible_criteria_sql})"
        )
        st.info(
        ...
        render_sinan_criterion_timeseries_chart(
            table,
            exprs,
            temporal_etiology_where,
            key_prefix="sinan_detailed_etiology_confirmed",
            fixed_stratum="Casos confirmados",
            criterion_codes=eligible_criterion_codes,
            scope_label=temporal_etiology_group.lower(),
            population_note=(
        ...
        st.warning(
            "CLASSI_FIN, CON_DIAGES e CRITERIO precisam estar presentes para estratificar a série temporal "
            "entre meningites bacterianas e assépticas."
        )
    st.divider()
    ...
    if not classi_code or not con_code or not con_label:
        st.warning(
            "A análise etiológica detalhada exige que CLASSI_FIN e CON_DIAGES sejam detectados. "
    ...
    diagnosis_specs = [
        ...
        ("Meningite meningocócica com meningococcemia", "03", "sinan_bacteriologia_casos_meningococica_com_meningococcemia.csv", False),
    ]
    tabs = st.tabs([label for label, _, _, _ in diagnosis_specs])
    for tab, (diagnosis_label, diagnosis_code, filename, include_bacteria) in zip(tabs, diagnosis_specs):
        with tab:
            diagnosis_cases = query_sinan_etiology_cases(
                table,
                exprs,
                confirmed_where,
                [diagnosis_code],
                include_bacteria_specification=include_bacteria,
            )
            detail_note = (
                " Para este grupo, `CLA_ME_BAC_CODIGO` e `bacteria_especificada` detalham o tipo de bactéria."
                if include_bacteria else ""
            )
            st.caption(
    ...
    st.markdown("#### Especificação de meningite asséptica")
    ass_label = exprs.get("cla_me_ass_label")
    ass_code = exprs.get("cla_me_ass_code")
    aseptic_where = append_clause(confirmed_where, f"{con_code} = '07'")
    total_assepticas = count_rows(table, aseptic_where)
    if not ass_label:
        st.warning(
            "CLA_ME_ASS não foi detectado. É possível identificar CON_DIAGES=07, mas não discriminar os agentes do Quadro III."
        )
        ...
        if not aseptic_df.empty:
            ...
            aseptic_df["pct_confirmados"] = (
                (aseptic_df["n"] / total_confirmados * 100).round(2)
                if total_confirmados
                else np.nan
            )
            aseptic_df["texto"] = [
                f"{br_int(n)} ({br_pct(pct_g)} do grupo; {br_pct(pct_c)} dos confirmados)"
                for n, pct_g, pct_c in zip(
                    aseptic_df["n"], aseptic_df["pct"], aseptic_df["pct_confirmados"]
                )
            ]
            ass_coverage_df = query_field_coverage(table, ass_code, aseptic_where) if ass_code else pd.DataFrame()
            ass_coverage_text = coverage_subtitle_from_df(ass_coverage_df) if not ass_coverage_df.empty else ""
            fig_aseptic = px.bar(
                aseptic_df,
                x="n",
                y="categoria",
                orientation="h",
                text="texto",
                title="Especificação de meningite asséptica"
                + (f"<br><sup>{ass_coverage_text}</sup>" if ass_coverage_text else ""),
                labels={
                    "categoria": "Agente (CLA_ME_ASS - Quadro III)",
                    "n": "Casos",
                    "pct": "% do grupo",
                    "pct_confirmados": "% dos confirmados",
                },
    ...
    if not ass_code:
        ...
        aseptic_option = st.selectbox(
            "Etiologia viral/asséptica (Quadro III)",
            list(SINAN_CLA_ME_ASS.values()),
            key="sinan_detailed_aseptic_case_etiology",
        )
        aseptic_selected_code = next(
            code for code, label in SINAN_CLA_ME_ASS.items() if label == aseptic_option
        )
        aseptic_selected_where = append_clause(
            confirmed_where, f"({ass_code}) = {qstr(aseptic_selected_code)}"
        )
        aseptic_cases = query_sinan_etiology_cases(
            table,
            exprs,
            aseptic_selected_where,
            ["07"],
            include_aseptic_specification=True,
        )
        if aseptic_cases.empty:
            st.info(f"Nenhum caso de {aseptic_option.lower()} no recorte atual.")
        else:
            st.metric("Casos no recorte", format_int_br(len(aseptic_cases)))
            copyable_dataframe(aseptic_cases, width="stretch", hide_index=True)
            download_button(
                aseptic_cases,
                f"sinan_etiologia_casos_assepticos_{aseptic_selected_code}.csv",
                label="Baixar tabela de casos (CSV)",
                max_rows=len(aseptic_cases),
            )

    st.markdown("#### Especificação de meningite por outras etiologias")
    eti_label = exprs.get("cla_me_eti_label")
    eti_code = exprs.get("cla_me_eti_code")
    other_etiology_where = append_clause(confirmed_where, f"{con_code} = '08'")
    total_outras_etiologias = count_rows(table, other_etiology_where)
    if not eti_label:
        st.warning(
            "CLA_ME_ETI não foi detectado. É possível identificar CON_DIAGES=08, mas não discriminar as etiologias "
            "do Quadro IV."
        )
        ...
        if not other_etiology_df.empty:
            ...
            other_etiology_df["pct_confirmados"] = (
                (other_etiology_df["n"] / total_confirmados * 100).round(2)
                if total_confirmados
                else np.nan
            )
            other_etiology_df["texto"] = [
                f"{br_int(n)} ({br_pct(pct_g)} do grupo; {br_pct(pct_c)} dos confirmados)"
                for n, pct_g, pct_c in zip(
                    other_etiology_df["n"],
                    other_etiology_df["pct"],
                    other_etiology_df["pct_confirmados"],
                )
            ]
            eti_coverage_df = (
                query_field_coverage(table, eti_code, other_etiology_where)
                if eti_code
                else pd.DataFrame()
            )
            eti_coverage_text = (
                coverage_subtitle_from_df(eti_coverage_df) if not eti_coverage_df.empty else ""
            )
            fig_other_etiology = px.bar(
                other_etiology_df,
                x="n",
                y="categoria",
                orientation="h",
                text="texto",
                title="Especificação de meningite por outras etiologias"
                + (f"<br><sup>{eti_coverage_text}</sup>" if eti_coverage_text else ""),
                labels={
                    "categoria": "Etiologia (CLA_ME_ETI - Quadro IV)",
                    "n": "Casos",
                    "pct": "% do grupo",
                    "pct_confirmados": "% dos confirmados",
                },
            ...
            render_plotly_chart(
                fig_other_etiology,
                calc_title="Especificação de meningite por outras etiologias",
            )
            st.caption(
            ...
            download_button(
                other_etiology_df,
                "sinan_etiologia_especificacao_outras_etiologias.csv",
            )

    st.markdown("#### Tabelas discriminatórias por diagnóstico de meningite por outra etiologia")
    st.caption(
        "Selecione uma etiologia do Quadro IV. A tabela restringe CLASSI_FIN=1 e CON_DIAGES=08, mostra o código e a "
        "descrição de CLA_ME_ETI e mantém o critério de confirmação ao lado. Opções sem casos continuam disponíveis."
    )
    if not eti_code:
        st.warning("CLA_ME_ETI não foi detectado; não é possível criar as tabelas discriminatórias do Quadro IV.")
    else:
        other_etiology_option = st.selectbox(
            "Etiologia da meningite por outra etiologia (Quadro IV)",
            list(SINAN_CLA_ME_ETI.values()),
            key="sinan_detailed_other_etiology_case_type",
        )
        other_etiology_selected_code = next(
            code for code, label in SINAN_CLA_ME_ETI.items() if label == other_etiology_option
        )
        other_etiology_selected_where = append_clause(
            confirmed_where, f"({eti_code}) = {qstr(other_etiology_selected_code)}"
        )
        other_etiology_cases = query_sinan_etiology_cases(
            table,
            exprs,
            other_etiology_selected_where,
            ["08"],
            include_other_etiology_specification=True,
        )
        if other_etiology_cases.empty:
            st.info(f"Nenhum caso de {other_etiology_option.lower()} no recorte atual.")
        else:
            st.metric("Casos no recorte", format_int_br(len(other_etiology_cases)))
            copyable_dataframe(other_etiology_cases, width="stretch", hide_index=True)
            download_button(
                other_etiology_cases,
                f"sinan_etiologia_casos_outras_etiologias_{other_etiology_selected_code}.csv",
                label="Baixar tabela de casos (CSV)",
                max_rows=len(other_etiology_cases),
            )
...
def render_spreadsheet_analysis_tab(
    ...
    def group_where(selection: str) -> str:
        ...
        if selection == "Casos confirmados" and classi_code:
            where_sql = append_clause(where_sql, f"{classi_code} = '1'")
        elif selection == "Casos descartados" and classi_code:
            where_sql = append_clause(where_sql, f"{classi_code} = '2'")
        return where_sql
    ...
    st.info(
        ...
        "Ela compara cada método com os códigos do Dicionário de Dados SINAN NET — "
        "Meningite v5.0. Valores vazios e códigos não previstos são erros de preenchimento; códigos como "
        "‘não realizado’, ‘ignorado’ ou ‘nenhum agente’ continuam sendo preenchimentos válidos quando constam no quadro "
        "correspondente. Em campos numéricos, a ausência do zero à esquerda é apenas uma propriedade do formato e não um erro."
    )
    st.markdown("### Verificação de completude de colunas")
    c1, c2 = st.columns(2)
    with c1:
        completeness_group = st.selectbox(
            "Estrato de casos",
            group_options,
            key="sinan_sheet_completeness_group",
        )
    with c2:
        selected_method = st.selectbox(
            "Coluna/método a verificar",
            SINAN_SHEET_COMPLETENESS_OPTIONS,
            key="sinan_sheet_completeness_method",
        )
    completeness_where = group_where(completeness_group)
    selected_spec = SINAN_DIAGNOSTIC_METHOD_SPECS[selected_method]
    selected_expr = _sinan_sheet_method_expr(exprs, selected_method)
    st.caption(
        f"Fonte normativa: {selected_spec.get('dictionary_source')}. O gráfico agrega o método em três situações: "
        "preenchida corretamente, vazia ou preenchida com valor fora do dicionário."
    )
    if not selected_expr:
        st.warning(
            f"Não foi localizada a coluna necessária para ‘{selected_method}’ na base carregada. "
            f"Referência esperada: {selected_spec.get('expr_key')}."
        )
    else:
        completeness_df = query_sinan_sheet_column_completeness(
            table, exprs, completeness_where, selected_method
        )
        if completeness_df.empty:
            st.info("Não há registros elegíveis para a coluna e o estrato selecionados.")
        else:
            completeness_df = completeness_df.copy()
            completeness_df["texto"] = [
                f"{format_int_br(int(n))} ({format_pct_br(p)})"
                for n, p in zip(completeness_df["n"], completeness_df["pct_total_elegivel"])
            ]
            ...
            st.caption(
                "As três categorias são mutuamente exclusivas e usam o mesmo denominador: todos os casos elegíveis do "
                "estrato. Os nomes de bactérias ou vírus previstos nos quadros são usados apenas para validar o código; "
                "eles não são tratados como categorias deste gráfico de completude."
            )
            complete_export = completeness_df.drop(columns=["texto"], errors="ignore")
            copyable_dataframe(complete_export, width="stretch", hide_index=True)
            download_button(
                complete_export,
                f"sinan_planilha_completude_{safe_filename(selected_method)}.csv",
                max_rows=len(complete_export),
            )

            error_df = query_sinan_sheet_column_errors(
                table, exprs, completeness_where, selected_method
            )
            st.markdown("**Casos com erro de preenchimento**")
            if error_df.empty:
                st.success("Nenhuma célula vazia ou fora do dicionário foi encontrada no recorte selecionado.")
            else:
                st.caption(
                    "A tabela contém todos os registros com célula vazia ou código não previsto no dicionário. "
                    "A linha é calculada antes dos filtros. Em tabelas DuckDB e em relações reordenadas, ela representa "
                    "a sequência devolvida pela relação, não uma chave permanente; use também NU_NOTIFIC/NM_PACIENT para localizar o caso."
                )
                copyable_dataframe(error_df, width="stretch", hide_index=True)
                download_button(
                    error_df,
                    f"sinan_planilha_erros_{safe_filename(selected_method)}.csv",
                    label="Baixar todos os casos com erro (CSV)",
                    max_rows=len(error_df),
                )

            st.markdown("**Detalhamento de todas as possibilidades previstas no dicionário**")
            method_values_df = query_sinan_sheet_method_value_distribution(
                table, exprs, completeness_where, selected_method
            )
            if method_values_df.empty:
                st.info("Não foi possível detalhar o domínio oficial desta coluna no recorte atual.")
            else:
                method_values_df = method_values_df.copy()
                method_values_df["texto"] = [
                    f"{format_int_br(int(n))} ({format_pct_br(pct)})"
                    for n, pct in zip(
                        method_values_df["n"], method_values_df["pct_total_elegivel"]
                    )
                ]
                method_category_order = method_values_df["categoria"].astype(str).tolist()
                fig_method_values = px.bar(
                    method_values_df,
                    x="n",
                    y="categoria",
                    orientation="h",
                    ...
                    text="texto",
                    title=(
                        f"Possibilidades de preenchimento de {selected_method} — "
                        f"{completeness_group.lower()}"
                    ),
                    labels={
                        "categoria": "Código e resultado previstos no dicionário",
                        "n": "Casos elegíveis",
                        "status_preenchimento": "Situação",
                        "pct_total_elegivel": "% entre todos os casos elegíveis",
                        "pct_entre_validamente_preenchidos": "% entre células válidas",
                        "total_elegivel": "Total elegível",
                        "total_validamente_preenchido": "Total validamente preenchido",
                    },
                    hover_data={
                        "texto": False,
                        "codigo": True,
                    ...
                    category_orders={
                        "status_preenchimento": ["Válida", "Vazia", "Fora do dicionário"]
                    },
                    color_discrete_map={
                        "Válida": PLOTLY_DEFAULT_BLUE,
                        "Vazia": "#7F7F7F",
                        "Fora do dicionário": "#D62728",
                    },
                )
                fig_method_values.update_yaxes(
                    categoryorder="array",
                    categoryarray=list(reversed(method_category_order)),
                    automargin=True,
                    tickfont={"size": 11},
                )
                fig_method_values.update_xaxes(automargin=True)
                fig_method_values.update_layout(
                    height=max(760, 48 * len(method_category_order) + 220),
                    margin={"l": 340, "r": 190, "t": 110, "b": 80},
                )
                fig_method_values.update_traces(textposition="outside", cliponaxis=False)
                render_plotly_chart(
                    fig_method_values,
                    calc_title=f"Possibilidades previstas para {selected_method}",
                )
                st.caption(
                    f"Domínio oficial: {selected_spec.get('dictionary_source')}. Todas as opções do quadro aplicável "
                    "são mantidas no eixo e na tabela, inclusive quando a contagem é zero. O percentual dos rótulos usa "
                    "todos os casos elegíveis; a tabela também apresenta o percentual restrito às células validamente "
                    "preenchidas. Para estes métodos laboratoriais, os domínios aplicáveis são os Quadros VII a XII."
                )
                method_values_export = method_values_df.drop(columns=["texto"], errors="ignore")
                copyable_dataframe(method_values_export, width="stretch", hide_index=True)
                download_button(
                    method_values_export,
                    f"sinan_planilha_possibilidades_{safe_filename(selected_method)}.csv",
                    label="Baixar detalhamento de todas as possibilidades (CSV)",
                    max_rows=len(method_values_export),
                )
    ...
    st.info(
        ...
        "(pneumococo). Categorias amplas, vazias, ignoradas, não realizadas ou sem agente são excluídas. Códigos de "
        "colunas numéricas são padronizados antes da comparação, preservando o valor bruto nas tabelas de auditoria."
    )
    ...
    compatibility_df = query_sinan_bacterioscopy_compatibility(
        table, exprs, compatibility_where
    )
    if compatibility_df.empty:
        st.info(
            "Não há casos que atendam simultaneamente aos critérios estritos de preenchimento e precisão, "
            "ou alguma das colunas LAB_BCLIQU/LAB_CTLIQU/LAB_CTSANG/LAB_PCLIQU/CON_DIAGES não foi detectada."
        )
    else:
        comparator_specs = [
            ("Cultura do líquor", "compat_cultura_liquor"),
            ("Cultura do sangue", "compat_cultura_sangue"),
            ("PCR do líquor", "compat_pcr_liquor"),
        ]
        compatibility_rows: List[Dict[str, object]] = []
        total_compat = len(compatibility_df)
        for comparator_label, status_col in comparator_specs:
            counts = compatibility_df[status_col].value_counts(dropna=False)
            for status in ["Compatível", "Incompatível"]:
                n = int(counts.get(status, 0))
                compatibility_rows.append({
                    "comparador": comparator_label,
                    "situacao": status,
                    "n": n,
                    "denominador": total_compat,
                    "pct": round(100.0 * n / total_compat, 2) if total_compat else np.nan,
                })
        compatibility_plot = pd.DataFrame(compatibility_rows)
        compatibility_plot["texto"] = [
            f"{format_int_br(int(n))} ({format_pct_br(p)})"
            for n, p in zip(compatibility_plot["n"], compatibility_plot["pct"])
        ]
        fig_compat = px.bar(
            compatibility_plot,
            x="comparador",
            ...
            barmode="group",
            text="texto",
            title="Compatibilidade etiológica com o achado da bacterioscopia do líquor",
            labels={"comparador": "Exame/diagnóstico comparado", "n": "Casos", "situacao": "Compatibilidade", "pct": "% dos casos elegíveis"},
            hover_data={"texto": False, "pct": ":.2f", "denominador": True},
            color_discrete_map={"Compatível": "#2CA02C", "Incompatível": "#D62728"},
        )
        fig_compat.update_traces(textposition="outside", cliponaxis=False)
        render_plotly_chart(fig_compat, calc_title="Compatibilidade da bacterioscopia com culturas e PCR")
        st.caption(
            f"Denominador comum: {format_int_br(total_compat)} caso(s) com todos os exames e diagnóstico final específicos. "
            "Cada barra compara o agente inferido pela morfologia da bacterioscopia com o agente do respectivo exame."
        )
        detail_export = compatibility_df.drop(columns=["formato_origem", "origem"], errors="ignore")
        copyable_dataframe(detail_export, width="stretch", hide_index=True)
        download_button(
            detail_export,
            "sinan_planilha_compatibilidade_bacterioscopia_culturas_pcr.csv",
            label="Baixar casos da comparação (CSV)",
            max_rows=len(detail_export),
        )
...
def render_methodology():
    ...
    st.markdown(
        ...
        7. Use **Análise etiológica detalhada** para reproduzir a conclusão diagnóstica, detalhar outras bactérias, meningites assépticas e outras etiologias pelos Quadros II, III e IV e revisar, caso a caso, o critério de confirmação.
    ...
    st.markdown("### Fidelidade dos valores, códigos sentinela e tetos de plausibilidade")
    st.markdown(
        """
        O observatório adota **fidelidade bruta por padrão**: o conteúdo do CSV, Parquet ou DuckDB não é corrigido,
        sobrescrito, truncado, arredondado, winsorizado ou imputado. Para desenhar um eixo quantitativo, o texto do
        campo precisa ser interpretado como número, mas sua magnitude registrada é mantida. Assim, um valor `999`
        continua aparecendo como 999 enquanto o modo opcional estiver desligado.

        **Código sentinela** é um número reservado que pode representar uma situação administrativa, como “ignorado”,
        “não informado” ou “não se aplica”, em vez de uma medida biológica. Como essa interpretação depende da versão e
        do dicionário da base, o painel não a aplica automaticamente. No raincloud de glicose não existe mais teto máximo
        fixo de 999 mg/dL: todo valor numericamente interpretável permanece no gráfico e nas estatísticas.

        **Teto de plausibilidade** é um limite analítico usado para sinalizar um valor que merece revisão, sem afirmar
        que o registro está errado. Exemplo: neutrófilos são interpretados como percentual; `LAB_NEUTRO = 135` ultrapassa
        o teto de 100%. Já `LAB_LEUCO = 60.000 céls/mm³` ultrapassa o teto provisório de 50.000 definido no código.
        Esses valores podem refletir erro de preenchimento, unidade diferente, campo com outra semântica ou um extremo
        real; por isso permanecem visíveis no modo padrão.

        Na área do LCR existe o controle **“Estratificar a análise aplicando códigos sentinela e tetos de
        plausibilidade”**, desligado inicialmente. Quando ativado, sentinelas e valores acima do teto são convertidos em
        ausentes **somente na consulta analítica**: o arquivo-fonte permanece intacto. O painel então gera uma tabela com
        a linha de origem, identificadores disponíveis, parâmetro, valor bruto, teto e motivo da sinalização. Limites de
        sistema (por exemplo, um campo que admite no máximo 99) são mostrados para auditoria, mas não são excluídos apenas
        por coincidirem com esse limite técnico.
        """
    )
    st.markdown("### Raincloud plots half-eye da idade e dos parâmetros liquóricos")
    st.markdown(
        "Os rainclouds combinam a meia densidade, os intervalos P2,5–P97,5 e Q1–Q3, a mediana e as observações. "
        "Losangos marcam P2,5/P97,5, quadrados marcam P25/P75 e o ponto branco marca P50; o cursor e a tabela abaixo "
        "fornecem os valores exatos, e a tabela de frequências registra a repetição de cada valor. No gráfico da idade, "
        "o SINAN compara confirmados e descartados; SIM e CIHA representam o conjunto definido pelos filtros, pois não "
        "possuem equivalente a `CLASSI_FIN`. São usados todos os registros com idade válida entre 0 e 130 anos.\n\n"
    ...
    st.markdown(
        ...
        "v5.0. Colunas numéricas podem perder o zero à esquerda durante a conversão para Parquet; nesses casos, 1 e 7 "
        "são padronizados como 01 e 07. Em colunas textuais, espaços, decimais e pontuação não são corrigidos. O gráfico "
    ...
    st.markdown(
        ...
        "registros com `LAB_PUNCAO = 1` e separa duas situações mutuamente exclusivas: células não preenchidas e "
        "preenchidas. Esta verificação é literal e não usa faixas da literatura para reclassificar o conteúdo. "
        "Os rainclouds calculam somente o "
